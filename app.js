...
// ====================================================================
class SyncManager {
  constructor({ mode, onStateUpdate, onPlayerAction, onPlayerHello, onStatusChange, onPeerListChange, onError, onMapImage, onTokenPos, onSoundData }) {
    this.mode = mode;
    this.peer = null;
    this.roomCode = null;
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4, maxHeight: 300, overflowY: 'auto' }}>
          ...
          {list.map(s => (
            <div key={s.id} style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 11, padding: '4px 6px', background: 'var(--bg-0)', borderRadius: 3 }}>
              ...
              <button className="btn sm" onClick={() => onPlay(s.id)} title="Play for the table">▶</button>
  ...
  useEffect(() => {
    ...
    if (auth.mode === 'dm') {
      ...
      sync.onStateUpdate = (payload) => {
        if (!fired) { fired = true; onFirstState(); }
