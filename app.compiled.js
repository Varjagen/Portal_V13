...
async function migrateLocalStorageToIDB() {
  try {
    ...
    if (existingIDB) return {
      migrated: false,
      reason: 'idb-already-has-data'
    };
    // Try v6 keys
    let raw = null,
      source = null;
...
const CONDITIONS = ['Blinded', 'Charmed', 'Deafened', 'Frightened', 'Grappled', 'Incapacitated', 'Invisible', 'Paralyzed', 'Petrified', 'Poisoned', 'Prone', 'Restrained', 'Stunned', 'Unconscious', 'Exhausted', 'Concentrating', 'Raging', 'Blessed', 'Hasted', 'Dead',
...
// v4 fix #19: Object presets
{
  ...
  id: 'builtin:reinforced_trap_door',
  name: 'Reinforced Trap Door',
  builtin: true,
  entity: {
    type: 'Object',
    name: 'Reinforced Trap Door',
...
// v4 fix #20: NPC presets
{
  ...
  id: 'builtin:npc_female_commoner',
  name: 'Female Commoner',
  builtin: true,
  entity: {
    type: 'NPC',
    name: 'Commoner (f)',
...
// ==========================================================
{
  ...
  id: 'builtin:sick_village_guard',
  name: 'Sick Village Guard',
  builtin: true,
  category: 'Humanoid',
  cr: '1/2',
  entity: {
...
function rollDiceMixed(counts, peerId, peerName) {
  ...
  if (groups.length === 0) {
    ...
    groups.push({
      die: 20,
      results: [1 + Math.floor(Math.random() * 20)]
    });
  }
  const total = groups.reduce((s, g) => s + g.results.reduce((a, r) => a + r, 0), 0);
...
// v7.1 #4: Polygon clipping for the freeform-polygon eraser.
// Implementation is a convex-clip Sutherland–Hodgman applied to the
// *complement* of each edge of the cut polygon, producing A - B.
//
// Key insight: clipping subject A against HALF-PLANE H gives A ∩ H.
// The complement of a polygon is the intersection of the half-planes
// on the OUTER side of each of its edges. So for each clip edge, we
// clip subject against "OUTSIDE of that edge" → one piece of the
// difference. The UNION of all those pieces is A - B (for convex B).
//
// For concave clip polygons we decompose via ear-clipping
// triangulation into triangles, then subtract each triangle from
// every remaining piece in sequence.
//
// For the TTRPG eraser use case (user drags a rough polygon to erase
// chunks of a wall), this is robust and correct.
...
function polygonSubtract(subject, clip) {
  ...
  if (subject.length === clip.length) {
    let allSame = true;
    for (let i = 0; i < subject.length; i++) {
      if (Math.abs(subject[i][0] - clip[i][0]) > 1e-4 || Math.abs(subject[i][1] - clip[i][1]) > 1e-4) {
        allSame = false;
        break;
      }
    }
  ...
  let anyCross = false,
    anySubjectInClip = false;
  for (const p of subject) {
    if (pointInPoly(p[0], p[1], clip)) {
      anySubjectInClip = true;
      break;
    }
  }
  outer: for (let i = 0; i < subject.length; i++) {
    const a = subject[i],
      b = subject[(i + 1) % subject.length];
    for (let j = 0; j < clip.length; j++) {
      const c = clip[j],
        d = clip[(j + 1) % clip.length];
  ...
  if (pieces.length > 0) {
    ...
    if (Math.abs(totalArea - originalArea) < 0.5) {
      ...
      let sxMin = Infinity,
        ...
        syMax = -Infinity;
      for (const p of subject) {
        if (p[0] < sxMin) sxMin = p[0];
        if (p[0] > sxMax) sxMax = p[0];
        if (p[1] < syMin) syMin = p[1];
        if (p[1] > syMax) syMax = p[1];
      }
      let cxMin = Infinity,
        cyMin = Infinity,
        cxMax = -Infinity,
        cyMax = -Infinity;
      for (const p of clip) {
        if (p[0] < cxMin) cxMin = p[0];
        if (p[0] > cxMax) cxMax = p[0];
        if (p[1] < cyMin) cyMin = p[1];
        if (p[1] > cyMax) cyMax = p[1];
      }
...
const makeDefaultState = () => {
  ...
  return {
    ...
    currentMapId: mapId,
    forcedView: null,
    // legacy (global push-view) — kept for back-compat
    forcedViewPerPeer: {},
    // v3: per-peer push (peerId -> { mapId })
    playerMapOverride: null,
    // player-chosen map when not forced
    claims: {},
    // v2 claim record
    entityOrder: [],
    reminders: {},
    // per-user private reminder tokens
    mapScale: 1.0,
    // global DM-controlled scale
    // v3 additions:
    timeOfDay: 0,
    // 0 = bright day, 1 = deep night; smooth scalar
    blockZones: {},
    // mapId -> [{id, x, y, w, h}]
    tokenPresets: {},
    // DM-defined presets keyed by id: { id, name, entity: partial }
...
function migrateState(raw) {
  ...
  const missing = existingIds.filter(id => !seen.has(id)).sort((a, b) => (state.entities[a].name || '').localeCompare(state.entities[b].name || ''));
  ...
  {
    const cleanGroups = {};
    const incoming = state.tokenGroups && typeof state.tokenGroups === 'object' ? state.tokenGroups : {};
    for (const [id, g] of Object.entries(incoming)) {
      if (!g || typeof g !== 'object') continue;
      if (!g.id || !g.mapId || typeof g.name !== 'string') continue;
      const validMembers = Array.isArray(g.memberIds) ? g.memberIds.filter(tid => {
...
function reducer(state, action) {
  switch (action.type) {
    ...
    case 'REPLACE':
      {
        ...
        for (const [id, m] of Object.entries(incomingMaps)) {
          if (m?.imageUrl === IMG_SENTINEL && currentMaps[id]?.imageUrl && currentMaps[id].imageUrl !== IMG_SENTINEL) {
            mergedMaps[id] = {
              ...m,
              imageUrl: currentMaps[id].imageUrl
            };
          } else {
            mergedMaps[id] = m;
          }
    ...
    case 'ENTITY_UPSERT':
      {
        ...
        const entityOrder = isNew ? [...(state.entityOrder || []), action.entity.id] : state.entityOrder || [];
        return {
          ...state,
          entities,
          entityOrder
    ...
    case 'ENTITY_HP_ADJUST':
      {
        ...
        if (cur === 0) {
          ...
          let targetCond;
          if (e.type === 'PC') targetCond = 'Unconscious';else if (e.type === 'Object') targetCond = 'Broken';else targetCond = 'Dead';
    ...
    case 'ENTITY_TOGGLE_CONDITION':
      {
        ...
        return {
          ...
          entities: {
            ...
            [action.id]: {
              ...e,
              conditions: has ? e.conditions.filter(c => c !== action.condition) : [...e.conditions, action.condition]
            }
          }
        };
      }
    ...
    case 'MAP_DELETE':
      {
        ...
        // reparent children
        const maps = Object.fromEntries(Object.entries(rest).map(([k, v]) => [k, v.parentId === action.id ? {
          ...v,
          parentId: null
        } : v]));
        return {
    ...
    case 'TOKEN_PLACE':
      {
        ...
        const existing = Object.values(state.tokens).find(t => t.entityId === action.token.entityId && t.mapId === action.token.mapId);
        if (existing) return state;
        return {
          ...state,
          tokens: {
            ...state.tokens,
            [action.token.id]: action.token
          }
        };
      }
    case 'TOKEN_MOVE':
    ...
    // v7 #1: Replace whole state from IDB hydration (or import). The
    // incoming state has already been migrated.
    case 'HYDRATE':
        ...
        if (groups && typeof groups === 'object') {
          ...
          for (const [gid, g] of Object.entries(groups)) {
            if ((g.memberIds || []).includes(action.id)) {
              nextGroups[gid] = {
                ...g,
                memberIds: g.memberIds.filter(x => x !== action.id)
              };
              groupsChanged = true;
            } else {
              nextGroups[gid] = g;
    ...
    case 'TOKEN_REVEAL_ALL_ON_MAP':
      {
        const tokens = Object.fromEntries(Object.entries(state.tokens).map(([k, t]) => [k, t.mapId === action.mapId ? {
          ...t,
          visible: action.visible
        } : t]));
        return {
    ...
    case 'PRESET_SAVE':
      return {
        ...
        presets: {
          ...state.presets,
          [action.preset.id]: action.preset
        }
      };
    case 'PRESET_DELETE':
      {
    ...
    case 'FORCED_VIEW':
      return {
        ...state,
        forcedView: action.forcedView
      };

    // Player map override
    case 'PLAYER_MAP_OVERRIDE':
      return {
        ...state,
        playerMapOverride: action.mapId
      };
    ...
    case 'CLAIM_PC':
      {
        ...
        const prev = nextClaims[action.peerId] || {
          pc: null,
          familiars: [],
          playerName: '',
          spectator: false
    ...
    case 'DM_UNCLAIM_PC':
      {
        ...
        const nextClaims = {};
        for (const [p, c] of Object.entries(state.claims)) {
          nextClaims[p] = c.pc === action.entityId ? {
            ...c,
            pc: null
          } : c;
        }
    ...
    case 'CLAIM_FAMILIAR':
      {
        ...
        for (const [p, c] of Object.entries(state.claims)) {
          nextClaims[p] = p !== action.peerId && c.familiars.includes(action.entityId) ? {
            ...c,
            familiars: c.familiars.filter(id => id !== action.entityId)
          } : c;
        }
        const prev = nextClaims[action.peerId] || {
          pc: null,
        ...
        const nextFamiliars = prev.familiars.includes(action.entityId) ? prev.familiars : [...prev.familiars, action.entityId];
        nextClaims[action.peerId] = {
          ...prev,
    ...
    case 'UNCLAIM_FAMILIAR':
      {
        ...
        return {
          ...
          claims: {
            ...
            [action.peerId]: {
              ...prev,
              familiars: prev.familiars.filter(id => id !== action.entityId)
            }
          }
        };
      }
    ...
    case 'REMINDER_DELETE':
      {
        ...
        return {
          ...
          reminders: {
            ...state.reminders,
            [peerId]: list.filter(r => r.id !== action.id)
          }
        };
      }
    ...
    case 'ENTITY_PATCH':
      {
        ...
        if (patch.stats) next.stats = {
          ...e.stats,
          ...patch.stats
        };
        if (patch.deathSaves) next.deathSaves = {
          ...e.deathSaves,
          ...patch.deathSaves
        };
    ...
    case 'LONG_REST':
      {
        ...
        const targetIds = Array.isArray(action.entityIds) ? action.entityIds : Object.values(state.entities).filter(e => e.type === 'PC' || e.type === 'Familiar').map(e => e.id);
    ...
    case 'TIME_OF_DAY_SET':
      return {
        ...state,
        timeOfDay: clamp(Number(action.value) || 0, 0, 1)
      };
    ...
    case 'FORCED_VIEW_PEER_SET':
      {
        const next = {
          ...(state.forcedViewPerPeer || {})
        };
        if (action.mapId == null) delete next[action.peerId];else next[action.peerId] = {
          mapId: action.mapId
        };
        return {
          ...state,
          forcedViewPerPeer: next
        };
      }
    ...
    case 'BLOCK_ZONE_DELETE':
      {
        ...
        return {
          ...
          blockZones: {
            ...state.blockZones,
            [mapId]: list.filter(z => z.id !== action.id)
          }
        };
      }
    ...
    case 'DRAWING_CLEAR_OWNER':
      {
        ...
        return {
          ...
          drawings: {
            ...(state.drawings || {}),
            [action.mapId]: list.filter(d => d.owner !== action.owner)
          }
        };
      }
    ...
    case 'HAZARD_DELETE':
      {
        ...
        return {
          ...
          hazards: {
            ...(state.hazards || {}),
            [action.mapId]: list.filter(h => h.id !== action.id)
          }
        };
      }
    ...
    case 'CLAIM_MIGRATE':
      {
        ...
        if (oldClaim) {
          ...
          claims[toPeerId] = {
            ...claims[toPeerId],
            playerId: playerId || claims[toPeerId].playerId || null
          };
        ...
        if (fromPeerId && fvpp[fromPeerId] && fromPeerId !== toPeerId) {
          fvpp[toPeerId] = fvpp[fromPeerId];
          delete fvpp[fromPeerId];
        }
        const reminders = {
          ...(state.reminders || {})
        };
        if (fromPeerId && reminders[fromPeerId] && fromPeerId !== toPeerId) {
          reminders[toPeerId] = reminders[fromPeerId];
          delete reminders[fromPeerId];
        }
        return {
          ...state,
          claims,
          entities,
          forcedViewPerPeer: fvpp,
          reminders
        };
      }
    ...
    case 'ENTITY_DUPLICATE':
      {
        ...
        if (idx === -1) order.push(newId);else order.splice(idx + 1, 0, newId);
        return {
          ...state,
          entities: {
            ...state.entities,
    ...
    case 'REMINDER_MOVE':
      {
        const peerId = action.peerId;
        const list = state.reminders[peerId] || [];
        const nextList = list.map(r => r.id === action.id ? {
          ...r,
          x: action.x,
          y: action.y
        } : r);
    ...
    case 'TOKEN_GROUP_CREATE':
      {
        ...
        const validMembers = (Array.isArray(memberIds) ? memberIds : []).filter(tid => state.tokens?.[tid]?.mapId === mapId);
        const group = {
          id,
          mapId,
    ...
    case 'TOKEN_GROUP_SET_MEMBERS':
      {
        ...
        const g = state.tokenGroups?.[id];
        if (!g) return state;
        const valid = (Array.isArray(memberIds) ? memberIds : []).filter(tid => state.tokens?.[tid]?.mapId === g.mapId);
        return {
          ...state,
          tokenGroups: {
            ...state.tokenGroups,
    ...
    case 'TOKEN_GROUP_ADD_MEMBERS':
      {
        ...
        const g = state.tokenGroups?.[id];
        if (!g) return state;
        const toAdd = (Array.isArray(tokenIds) ? tokenIds : []).filter(tid => state.tokens?.[tid]?.mapId === g.mapId);
...
class SyncManager {
  constructor({
    ...
    this.onPlayerHello = onPlayerHello; // v4 — identity migration
    this.onStatusChange = onStatusChange;
    this.onPeerListChange = onPeerListChange;
    this.onError = onError;
    // v7.2: auxiliary envelopes. Map image bytes, ephemeral token
    // positions, and sound audio bytes travel on dedicated channels
    // so the main state payload stays lean and responsive.
  ...
  async hostSession(roomCode) {
    ...
    try {
      ...
      this.peer.on('connection', conn => {
        ...
        conn.on('data', data => {
          if (data.type === 'player_action') this.onPlayerAction?.(data.payload, conn.peer);else if (data.type === 'hello') this.onPlayerHello?.(data, conn.peer);
        });
        conn.on('close', () => {
  ...
  async joinSession(roomCode, playerId, playerName) {
    ...
    try {
      ...
      this.peer.on('open', id => {
        ...
        conn.on('open', () => {
          this.setStatus('live');
          // v4: identify ourselves with the stable playerId so the DM
          // can migrate our previous claim to our new peer id.
          conn.send({
            type: 'hello',
            peerId: id,
            playerId,
        ...
        conn.on('data', data => {
          ...
          // v7.2: map image bytes arrive separately from state so the
          // claim modal can render before megabytes of PNG data cross
          // the wire. Cache in IDB and merge into state.
          else if (data.type === 'map_image') this.onMapImage?.(data.mapId, data.dataUrl);
          // v7.2: ephemeral token position update. Bypasses the full
          // state broadcast so dragging feels responsive.
          else if (data.type === 'token_pos') this.onTokenPos?.(data.tokenId, data.x, data.y, data.mapId);else if (data.type === 'sound_data') this.onSoundData?.(data.soundId, data.name, data.dataUrl);else if (data.type === 'kicked') {
            // v4: DM removed us. Show error, then close.
  ...
  kickPeer(peerId, reason) {
    ...
    try {
      conn.send({
        type: 'kicked',
        reason: reason || 'The DM has removed you from the session.'
      });
    } catch {}
...
function computePlayerVisionSources(state, mapId, ownedEntityIds, timeOfDay = 0, alwaysDark = false) {
  ...
  let lightMul;
  if (effectiveTod >= 0.95) lightMul = 1.0;else if (effectiveTod >= 0.70) lightMul = 1.25;else lightMul = 1.75; // dusk/dawn band
...
function filterStateForPlayer(state, peerId) {
  ...
  const effectiveMapId = state.forcedViewPerPeer?.[peerId]?.mapId || state.forcedView?.mapId || state.currentMapId;
  ...
  if (visionActive) {
    // Compute the vision sources this player has on this map.
    const sources = computePlayerVisionSources(state, effectiveMapId, ownedIds, tod, mapAlwaysDark);
    // For each token on the active map, keep it only if it's within
    // at least one vision radius OR it's an owned entity.
    const cutTokens = {};
    for (const [k, t] of Object.entries(visibleTokens)) {
      if (t.mapId !== effectiveMapId) {
        cutTokens[k] = t;
        continue;
      }
      const ent = state.entities[t.entityId];
      if (!ent) continue;
      if (ownedIds.has(ent.id)) {
        cutTokens[k] = t;
        continue;
      }
      // v6 fix #4: Labels now follow the same vision rules as creatures.
      // If you can't see the church at night, you don't see its label.
      // In-radius check: distance(token, source) <= source.radius
      let visible = false;
    ...
    // Replace the visible-tokens map with the post-cutoff set.
    for (const k of Object.keys(visibleTokens)) delete visibleTokens[k];
    Object.assign(visibleTokens, cutTokens);
  }
...
function AuthScreen({
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, "The Plague's Call"), /*#__PURE__*/React.createElement("div", {
    className: "auth-subtitle"
  }, "\u2014 a virtual tabletop for tales of rot and rust \u2014"), /*#__PURE__*/React.createElement("div", {
    className: "auth-tab-row"
  }, /*#__PURE__*/React.createElement("div", {
    className: `auth-tab ${tab === 'dm' ? 'active' : ''}`,
    onClick: () => {
      setTab('dm');
      setError('');
    }
  }, "\u2694 Dungeon Master"), /*#__PURE__*/React.createElement("div", {
    className: `auth-tab ${tab === 'player' ? 'active' : ''}`,
    onClick: () => {
      setTab('player');
      setError('');
    }
  }, "\u2302 Player")), tab === 'dm' ? /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    className: "auth-field"
  }, /*#__PURE__*/React.createElement("label", null, "Passphrase"), /*#__PURE__*/React.createElement("input", {
    type: "password",
    value: password,
    onChange: e => setPassword(e.target.value),
    placeholder: "Enter the arcane word\u2026",
    autoFocus: true,
  ...
  }, "Default passphrase: ", /*#__PURE__*/React.createElement("kbd", null, "dragon"), " \u2014 edit ", /*#__PURE__*/React.createElement("code", null, "DM_PASSWORD"), " in ", /*#__PURE__*/React.createElement("code", null, "app.js"))), /*#__PURE__*/React.createElement("div", {
    className: "auth-field"
  }, /*#__PURE__*/React.createElement("label", null, "Room Code (optional)"), /*#__PURE__*/React.createElement("input", {
    value: roomCode,
  ...
  }, "Share with players so they may join.")), error && /*#__PURE__*/React.createElement("div", {
    style: {
    ...
    onClick: handleDM
  }, "Open the Session"), /*#__PURE__*/React.createElement("div", {
    className: "hr"
    ...
    onClick: handleLocal
  }, "\u2690 Local-only mode (no sync)")) : /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    className: "auth-field"
  }, /*#__PURE__*/React.createElement("label", null, "Room Code"), /*#__PURE__*/React.createElement("input", {
    value: roomCode,
    onChange: e => setRoomCode(e.target.value),
    placeholder: "e.g. curse-of-strahd",
    autoFocus: true,
...
function TokenView({
  ...
  const showHpBar = entity.hp.max > 0 && (mode === 'dm' || PLAYER_HP_VISIBLE_TYPES.has(entity.type));
  ...
  const classes = ['token', !token.visible ? 'hidden-token' : '', isCurrent ? 'current-turn' : '', isSelected ? 'selected' : '', isMultiSelected ? 'multi-selected' : '', isMeasureStart ? 'measure-start' : '', isDraggingLocal ? 'dragging' : ''].filter(Boolean).join(' ');
  ...
  }, showHpBar && /*#__PURE__*/React.createElement("div", {
    className: "token-hp-bar"
  }, /*#__PURE__*/React.createElement("div", {
    className: `token-hp-fill ${hpClass}`,
    style: {
      width: `${hpPct}%`
    }
  })), /*#__PURE__*/React.createElement("div", {
    className: `token-shape ${typeClass}`,
    style: {
      '--color': entity.color
    }
  }, entity.imageUrl ? /*#__PURE__*/React.createElement("img", {
    src: entity.imageUrl,
    alt: "",
    className: "token-portrait",
    draggable: "false"
  }) : /*#__PURE__*/React.createElement("span", null, initial)), showLabel && /*#__PURE__*/React.createElement("div", {
    className: "token-label"
  }, entity.name), showLabel && (statusItems.length > 0 || sicknessLabel) && /*#__PURE__*/React.createElement("div", {
    className: "token-status-stack"
  }, statusItems.map(c => /*#__PURE__*/React.createElement("span", {
    key: c,
    className: "token-status-chip",
    title: c,
    style: {
      background: CONDITION_COLORS[c] || 'rgba(120,120,120,0.85)'
    }
  }, c)), sicknessLabel && /*#__PURE__*/React.createElement("span", {
    className: "token-status-chip sickness",
    title: "Sickness"
  }, /*#__PURE__*/React.createElement("em", null, sicknessLabel.toLowerCase())))));
}
...
function MapCanvas({
  ...
  onReminderUpsert,
  onReminderDelete,
  placingReminder = false,
  onPlaceReminderDone,
  hoveredTokenId,
  onTokenHoverChange,
  // v3:
  visionEnabled = false,
  // whether to dim the map where nothing sees
  visionSources = [],
  // [{ x, y, radius }] — in world pixels
  blockZones = [],
  // [{ id, x, y, w, h }] — in world pixels
  placingBlock = false,
  onPlaceBlockDone,
  onBlockUpsert,
  onBlockDelete,
  placingFreeBlock = false,
  onPlaceFreeBlockDone,
  // v6 #8 + #13: two new block modes — circle draw + eraser.
  placingCircleBlock = false,
  ...
  drawings = [],
  // [{id, type, ..., color, width, owner}]
  drawMode = null,
  // null | 'free' | 'line' | 'circle'
  drawColor = '#c9a34a',
  drawWidth = 3,
  drawOwner = null,
  // peerId or 'dm' — tags the drawing
  onDrawingUpsert,
  ...
  hazards = [],
  placingHazard = null,
  // null | 'fire' | 'flood' | 'cold' | 'acid' | 'fog' | 'difficult'
  ...
  const [viewport, setViewport] = useState(map?.viewport || {
    x: 0,
    y: 0,
    zoom: 1
  });
  ...
  const onMapImageLoad = e => {
    ...
    const OFF = 4000;
    setMapBounds(prev => prev.W === W && prev.H === H && prev.OFF === OFF ? prev : {
      W,
      H,
      OFF
    });
      ...
      if (poly && poly.length >= 3) {
        if (erasingBlockRef.current) {
          ...
          for (const z of blockZonesRef.current || []) {
            ...
            const untouched = pieces.length === 1 && pieces[0].length === subjectPoly.length && pieces[0].every((p, i) => Math.abs(p[0] - subjectPoly[i][0]) < 1e-3 && Math.abs(p[1] - subjectPoly[i][1]) < 1e-3);
  ...
  useEffect(() => {
    ...
    const onMove = e => {
      ...
      setDrawingNow(prev => {
        ...
        if (prev.type === 'line') return {
          ...prev,
          x1: world.x,
          y1: world.y
        };
        if (prev.type === 'circle') {
          const dx = world.x - prev.cx,
            dy = world.y - prev.cy;
          return {
            ...prev,
  ...
  useEffect(() => {
    ...
    const onMove = e => {
      const world = screenToWorld(e.clientX, e.clientY);
      const cx = drawRef.current?.cx,
        cy = drawRef.current?.cy;
  ...
  const eraseAtClient = useCallback((clientX, clientY) => {
    ...
    for (const z of blockZones) {
      ...
      if (z.type === 'poly' && Array.isArray(z.points) && z.points.length >= 3) {
        ...
        const dx = world.x - z.cx,
          dy = world.y - z.cy;
        hit = dx * dx + dy * dy <= z.r * z.r;
      } else {
        // Rect (legacy shape)
        hit = world.x >= z.x && world.x <= z.x + z.w && world.y >= z.y && world.y <= z.y + z.h;
      }
  ...
  const visibleTokens = useMemo(() => Object.values(tokens).filter(t => t.mapId === map?.id), [tokens, map?.id]);
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, map?.imageUrl ? /*#__PURE__*/React.createElement("img", {
    src: map.imageUrl,
    alt: map.name,
    className: "map-image",
    draggable: "false",
    onLoad: onMapImageLoad
  }) : null, visibleTokens.map(t => {
    const ent = entities[t.entityId];
    if (!ent) return null;
    const isOwned = ownedEntityIds ? ownedEntityIds.has(ent.id) : claimedEntityId === ent.id;
    return /*#__PURE__*/React.createElement(TokenView, {
      key: t.id,
      token: t,
      entity: ent,
  ...
  }), reminders.filter(r => r.mapId === map?.id).map(r => /*#__PURE__*/React.createElement("div", {
    key: r.id,
    ...
    className: "reminder-pin-body"
  }, "\u25C6"), /*#__PURE__*/React.createElement("div", {
    className: "reminder-pin-label"
  }, r.label))), (blockZones.length > 0 || drawingBlock || drawingPoly || drawingCircle) && /*#__PURE__*/React.createElement("svg", {
    className: "block-zone-layer",
    ...
    if (isPoly) {
      return /*#__PURE__*/React.createElement("polygon", _extends({
        key: z.id,
        points: z.points.map(([x, y]) => `${x},${y}`).join(' ')
      }, dmProps), /*#__PURE__*/React.createElement("title", null, "Double-click to delete"));
    }
    if (isCircle) {
      return /*#__PURE__*/React.createElement("circle", _extends({
        key: z.id,
  ...
  }), drawingBlock && mode === 'dm' && /*#__PURE__*/React.createElement("rect", {
    x: drawingBlock.x,
    y: drawingBlock.y,
    width: drawingBlock.w,
    height: drawingBlock.h,
    fill: "rgba(200,80,80,0.22)",
    stroke: "rgba(255,120,120,0.85)",
    strokeWidth: "2",
    strokeDasharray: "4 4"
  }), drawingPoly && mode === 'dm' && drawingPoly.length >= 2 && /*#__PURE__*/React.createElement("polyline", {
    ...
    strokeLinejoin: "round",
    strokeLinecap: "round"
  }), drawingCircle && mode === 'dm' && drawingCircle.r > 2 && /*#__PURE__*/React.createElement("circle", {
    cx: drawingCircle.cx,
    cy: drawingCircle.cy,
    r: drawingCircle.r,
    ...
    strokeWidth: "2",
    strokeDasharray: "4 4"
  })), (drawings.length > 0 || drawingNow) && /*#__PURE__*/React.createElement("svg", {
    ...
    if (d.type === 'free' && Array.isArray(d.points) && d.points.length >= 2) {
      return /*#__PURE__*/React.createElement("polyline", _extends({
        key: d.id,
    ...
    if (drawingNow.type === 'free' && drawingNow.points.length >= 2) {
      return /*#__PURE__*/React.createElement("polyline", _extends({
        points: drawingNow.points.map(([x, y]) => `${x},${y}`).join(' ')
      }, p));
    }
    ...
    return null;
  })()), hazards.length > 0 && /*#__PURE__*/React.createElement("svg", {
    className: "hazard-layer",
    xmlns: "http://www.w3.org/2000/svg",
    style: {
      position: 'absolute',
      left: -mapBounds.OFF,
    ...
    width: "10",
    height: "10",
    fill: "rgba(140,100,60,0.18)"
  }), /*#__PURE__*/React.createElement("line", {
    x1: "0",
    y1: "0",
    x2: "0",
    y2: "10",
    stroke: "rgba(180,130,70,0.6)",
    strokeWidth: "1.5"
    ...
    switch (h.hazardKind) {
      ...
      case 'flood':
        style = {
          fill: 'rgba(60,120,200,0.28)',
          stroke: 'rgba(100,160,230,0.8)',
          strokeWidth: 2
        };
        break;
      case 'cold':
        style = {
          fill: 'url(#hz-stipple-cold)',
          stroke: 'rgba(200,230,245,0.85)',
          strokeWidth: 1.5
        };
        break;
      case 'acid':
        style = {
          fill: 'rgba(110,180,70,0.3)',
          stroke: 'rgba(150,210,90,0.85)',
          strokeWidth: 2
        };
        break;
      case 'fog':
        style = {
          fill: 'rgba(180,180,190,0.45)',
          stroke: 'rgba(200,200,210,0.55)',
          strokeWidth: 1.5,
          filter: 'url(#hz-fog-blur)'
        };
        break;
      case 'difficult':
        style = {
          fill: 'url(#hz-hatch-difficult)',
          stroke: 'rgba(160,110,50,0.75)',
          strokeWidth: 1.5
        };
        break;
      default:
    ...
    return /*#__PURE__*/React.createElement("polygon", _extends({
      key: h.id
    }, baseProps, style, dmHandlers), /*#__PURE__*/React.createElement("title", null, h.hazardKind, h.visible === false ? ' (hidden)' : '', h.label ? ` — ${h.label}` : ''));
    ...
    return /*#__PURE__*/React.createElement("svg", {
      ...
      viewBox: `${-mapBounds.OFF} ${-mapBounds.OFF} ${mapBounds.W} ${mapBounds.H}`
    }, isRadius ? /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("circle", {
      cx: x0,
      cy: y0,
      r: distPx,
    ...
    return /*#__PURE__*/React.createElement("svg", {
      ...
      viewBox: `${-OFF} ${-OFF} ${W} ${H}`
    }, /*#__PURE__*/React.createElement("defs", null, visionSources.map((s, i) => {
      ...
      return /*#__PURE__*/React.createElement("radialGradient", {
        ...
        stopColor: "black",
        stopOpacity: "1"
      }, flickers && /*#__PURE__*/React.createElement("animate", {
      ...
      height: H,
      fill: "white"
    }), visionSources.map((s, i) => /*#__PURE__*/React.createElement("circle", {
      key: i,
      cx: s.x,
      cy: s.y,
      r: s.radius,
  ...
  })(), mode === 'dm' && visionSources.length > 0 && /*#__PURE__*/React.createElement("svg", {
    className: "vision-outlines",
    xmlns: "http://www.w3.org/2000/svg",
    style: {
      position: 'absolute',
      left: 0,
    ...
    strokeDasharray: "6 6",
    opacity: "0.55"
  })))), !map?.imageUrl && /*#__PURE__*/React.createElement("div", {
    className: "map-empty"
  }, /*#__PURE__*/React.createElement("div", {
    className: "glyph"
  }, "\u269C"), /*#__PURE__*/React.createElement("div", {
  ...
  }, mode === 'dm' ? 'The canvas awaits. Upload a map image to begin.' : 'The realm is shrouded in mist.')), /*#__PURE__*/React.createElement("div", {
    className: "canvas-overlay top-right"
  }, /*#__PURE__*/React.createElement("div", {
...
function EntityForm({
  ...
  const update = patch => setEntity(e => ({
    ...e,
    ...patch
  }));
  const updateStat = (stat, value) => setEntity(e => ({
    ...e,
    stats: {
      ...e.stats,
      [stat]: Number(value) || 0
  ...
  const updateHp = (key, value) => setEntity(e => ({
    ...e,
    hp: {
      ...e.hp,
      [key]: Number(value) || 0
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, ENTITY_TYPES.map(t => /*#__PURE__*/React.createElement("option", {
    key: t,
    value: t
  }, t))))), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", null, "Token Image ", /*#__PURE__*/React.createElement("span", {
    style: {
      color: 'var(--ink-mute)',
      fontWeight: 400,
      textTransform: 'none',
  ...
  }, "\u2014 optional; falls back to colored token")), /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      gap: 10,
      alignItems: 'center'
    ...
    alt: "",
    draggable: "false"
  }) : /*#__PURE__*/React.createElement("span", null, (entity.name || '?').slice(0, 1).toUpperCase())), /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
    ...
    type: "button",
    onClick: uploadImage
  }, "\u21E7 Upload image"), entity.imageUrl && /*#__PURE__*/React.createElement("button", {
  ...
  }, /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", null, "Color"), /*#__PURE__*/React.createElement("input", {
    type: "color",
    value: entity.color,
    onChange: e => update({
      color: e.target.value
    })
  })), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", null, "AC"), /*#__PURE__*/React.createElement("input", {
    type: "number",
    value: entity.ac,
    onChange: e => update({
      ac: Number(e.target.value) || 0
    })
  })), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", null, "Speed"), /*#__PURE__*/React.createElement("input", {
    type: "number",
    value: entity.speed,
    onChange: e => update({
      speed: Number(e.target.value) || 0
    })
  }))), isHpType && /*#__PURE__*/React.createElement("div", {
    className: "form-row-3"
  }, /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", null, "HP Current"), /*#__PURE__*/React.createElement("input", {
    type: "number",
    value: entity.hp.current,
    onChange: e => updateHp('current', e.target.value)
  })), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", null, "HP Max"), /*#__PURE__*/React.createElement("input", {
    type: "number",
    value: entity.hp.max,
    onChange: e => updateHp('max', e.target.value)
  })), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", null, "Init Bonus"), /*#__PURE__*/React.createElement("input", {
    type: "number",
    value: entity.initBonus,
    onChange: e => update({
      initBonus: Number(e.target.value) || 0
    })
  }))), entity.type === 'Object' && /*#__PURE__*/React.createElement("div", {
    className: "form-row-2"
  }, /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", null, "Rolls Initiative?"), /*#__PURE__*/React.createElement("label", {
    className: "toggle-row"
  }, /*#__PURE__*/React.createElement("input", {
    type: "checkbox",
    checked: !!entity.rollsInitiative,
    onChange: e => update({
      rollsInitiative: e.target.checked
    })
  }), /*#__PURE__*/React.createElement("span", null, entity.rollsInitiative ? 'Included in initiative' : 'Static object — skipped'))), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", null, "Init Bonus"), /*#__PURE__*/React.createElement("input", {
    type: "number",
    value: entity.initBonus,
    disabled: !entity.rollsInitiative,
    onChange: e => update({
      initBonus: Number(e.target.value) || 0
    })
  }))), ['PC', 'Monster', 'NPC', 'Familiar', 'Neutral Beast'].includes(entity.type) && /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", null, "Ability Scores"), /*#__PURE__*/React.createElement("div", {
    className: "form-row-6"
  }, ['str', 'dex', 'con', 'int', 'wis', 'cha'].map(s => /*#__PURE__*/React.createElement("div", {
    key: s,
    className: "stat-box"
  }, /*#__PURE__*/React.createElement("label", null, s.toUpperCase()), /*#__PURE__*/React.createElement("input", {
    type: "number",
  ...
  }, modFor(entity.stats[s]) >= 0 ? `+${modFor(entity.stats[s])}` : modFor(entity.stats[s])))))), /*#__PURE__*/React.createElement("div", {
    className: "form-row-2"
  }, /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", null, "Passive Perception"), /*#__PURE__*/React.createElement("input", {
    type: "number",
    value: entity.passivePerception,
    onChange: e => update({
      passivePerception: Number(e.target.value) || 0
    })
  })), entity.type === 'PC' && /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", null, "Level"), /*#__PURE__*/React.createElement("input", {
    type: "number",
    value: entity.level,
    onChange: e => update({
      level: Number(e.target.value) || 1
    })
  })), entity.type === 'Monster' && /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", null, "Challenge Rating"), /*#__PURE__*/React.createElement("input", {
    value: entity.cr,
    onChange: e => update({
      cr: e.target.value
    })
  })), entity.type === 'NPC' && /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", null, "Faction"), /*#__PURE__*/React.createElement("input", {
    value: entity.faction,
    onChange: e => update({
      faction: e.target.value
    })
  })), entity.type === 'Familiar' && /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", null, "Bonded To ", /*#__PURE__*/React.createElement("span", {
    style: {
      color: 'var(--ink-mute)',
      fontWeight: 400,
      textTransform: 'none',
  ...
  }, "\u2014 the master PC name, if any")), /*#__PURE__*/React.createElement("input", {
    value: entity.faction,
    onChange: e => update({
      faction: e.target.value
    }),
    placeholder: "e.g. Caelum the wizard"
  })), entity.type === 'Neutral Beast' && /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", null, "Nature"), /*#__PURE__*/React.createElement("input", {
    value: entity.role,
    onChange: e => update({
      role: e.target.value
    }),
    placeholder: "e.g. deer, forest spirit"
  })), entity.type === 'Object' && /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", null, "Kind"), /*#__PURE__*/React.createElement("input", {
    value: entity.role,
    onChange: e => update({
      role: e.target.value
    }),
    placeholder: "e.g. altar, chest, rune"
  }))), entity.type === 'PC' && /*#__PURE__*/React.createElement("div", {
    className: "form-row-2"
  }, /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", null, "Class"), /*#__PURE__*/React.createElement("input", {
    value: entity.class,
    onChange: e => update({
      class: e.target.value
    }),
    placeholder: "e.g. Wizard"
  })), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", null, "Player Name"), /*#__PURE__*/React.createElement("input", {
    value: entity.playerName,
    onChange: e => update({
      playerName: e.target.value
    })
  }))), ['PC', 'NPC', 'Monster', 'Neutral Beast', 'Familiar'].includes(entity.type) && /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", null, "Sickness ", /*#__PURE__*/React.createElement("span", {
    style: {
      color: 'var(--ink-mute)',
      fontWeight: 400,
      textTransform: 'none',
  ...
  }, "\u2014 DM-only setting; descriptor appears on tooltips and under the token")), /*#__PURE__*/React.createElement("div", {
    className: "sickness-picker"
  }, [0, 1, 2, 3].map(lvl => /*#__PURE__*/React.createElement("button", {
    key: lvl,
    type: "button",
    ...
    className: "sickness-num"
  }, lvl), /*#__PURE__*/React.createElement("span", {
    className: "sickness-label"
  }, lvl === 0 ? 'Healthy' : SICKNESS_DESCRIPTORS[lvl]))))), (entity.type === 'Monster' || entity.type === 'Neutral Beast') && /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", null, "Abilities / DM Notes"), /*#__PURE__*/React.createElement("textarea", {
    value: entity.abilities,
    onChange: e => update({
      abilities: e.target.value
    }),
    placeholder: "Multiattack, breath weapon, legendary actions\u2026"
  })), isPlayerFacing && /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", null, "Player-Visible Description ", /*#__PURE__*/React.createElement("span", {
    style: {
      color: 'var(--ink-mute)',
      fontWeight: 400,
      textTransform: 'none',
  ...
  }, "\u2014 shown to players when revealed / on hover")), /*#__PURE__*/React.createElement("textarea", {
  ...
  })), ['PC', 'Familiar', 'Monster', 'Neutral Beast', 'NPC'].includes(entity.type) && /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", null, "Vision ", /*#__PURE__*/React.createElement("span", {
    style: {
      color: 'var(--ink-mute)',
      fontWeight: 400,
      textTransform: 'none',
  ...
  }, "\u2014 darkvision + carried light (feet)")), /*#__PURE__*/React.createElement("div", {
    className: "form-row-2"
  }, /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", {
    style: {
      fontSize: 9
    }
  }, "Darkvision"), /*#__PURE__*/React.createElement("input", {
    type: "number",
    min: "0",
    step: "5",
    value: entity.darkvision || 0,
    onChange: e => update({
      darkvision: Number(e.target.value) || 0
    })
  })), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", {
    style: {
      fontSize: 9
    }
  }, "Light Radius"), /*#__PURE__*/React.createElement("input", {
    type: "number",
    min: "0",
    step: "5",
  ...
  })))), entity.type === 'Object' && /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", null, "Light Source ", /*#__PURE__*/React.createElement("span", {
    style: {
      color: 'var(--ink-mute)',
      fontWeight: 400,
      textTransform: 'none',
  ...
  }, "\u2014 objects may emit light (feet)")), /*#__PURE__*/React.createElement("input", {
    type: "number",
    min: "0",
    step: "5",
    value: entity.lightRadius || 0,
  ...
  })), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", null, "Conditions"), /*#__PURE__*/React.createElement("div", {
    className: "cond-grid"
  }, CONDITIONS.map(c => /*#__PURE__*/React.createElement("div", {
    key: c,
    className: `cond-chip ${entity.conditions.includes(c) ? 'active' : ''}`,
    onClick: () => update({
      conditions: entity.conditions.includes(c) ? entity.conditions.filter(x => x !== c) : [...entity.conditions, c]
    })
  }, c)))), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", null, "DM Notes ", /*#__PURE__*/React.createElement("span", {
    style: {
      color: 'var(--ink-mute)',
      fontWeight: 400,
      textTransform: 'none',
  ...
  }, "\u2014 never shown to players")), /*#__PURE__*/React.createElement("textarea", {
    value: entity.notes,
    onChange: e => update({
...
function EntitySidebar({
  ...
  const adjustHp = (id, delta) => dispatch({
    type: 'ENTITY_HP_ADJUST',
    id,
    delta
  });
  ...
  const deletePreset = id => {
    ...
    dispatch({
      type: 'TOKEN_PRESET_DELETE',
      id
    });
  };
  const allPresets = [...BUILTIN_TOKEN_PRESETS, ...Object.values(state.tokenPresets || {})];
  ...
  return /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
  ...
  }, /*#__PURE__*/React.createElement("span", null, "Bestiary"), /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      gap: 4,
      position: 'relative'
    ...
    onClick: newEntity
  }, "\uFF0B New"), showPresetMenu && /*#__PURE__*/React.createElement(BestiaryMenu, {
  ...
  }, ['All', 'PC', 'Monster', 'NPC', 'Familiar', 'Object', 'Label'].map(f => /*#__PURE__*/React.createElement("div", {
    key: f,
    className: `pill ${filter === f ? 'active' : ''}`,
    onClick: () => setFilter(f)
  }, f)), /*#__PURE__*/React.createElement("div", {
    className: `pill ${!showDead ? 'active' : ''}`,
    onClick: () => setShowDead(!showDead)
  }, showDead ? 'Hide dead' : 'Show dead'))), /*#__PURE__*/React.createElement("div", {
    className: "sidebar-section grow"
  }, /*#__PURE__*/React.createElement("div", {
    ...
    className: "glyph"
  }, "\u2726"), entitiesByOrder.length === 0 ? 'No entities yet. Forge one.' : 'No matching entities.'), filtered.map(e => {
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      onDragStart: ev => {
        ev.stopPropagation();
        onCardDragStart(ev, e);
      },
      onClick: ev => ev.stopPropagation(),
      title: "Drag to reorder or to place on map"
    }, "\u22EE\u22EE"), /*#__PURE__*/React.createElement("div", {
      className: `entity-swatch ${swatchClass}`,
      style: {
        background: e.color
      }
    }), /*#__PURE__*/React.createElement("div", {
      ...
      className: "mono"
    }, e.type === 'PC' ? `L${e.level} ${e.class || ''}` : e.type === 'Monster' ? `CR ${e.cr}` : e.role || 'NPC'), /*#__PURE__*/React.createElement("span", {
      className: `entity-hp ${hpClass} mono`
    }, e.hp.current, "/", e.hp.max), /*#__PURE__*/React.createElement("span", {
      className: "mono",
      style: {
        color: 'var(--ink-mute)'
      }
    }, "AC ", e.ac))), onMap && /*#__PURE__*/React.createElement("button", {
      className: `eye-btn ${onMap.visible ? 'on' : 'off'}`,
      onClick: ev => {
        ev.stopPropagation();
        toggleVisibility(onMap);
      },
      title: onMap.visible ? 'Visible to players — click to hide' : 'Hidden from players — click to reveal'
    }, onMap.visible ? '👁' : '⦿'), /*#__PURE__*/React.createElement("div", {
    ...
    }, "\u270E"))), expanded && /*#__PURE__*/React.createElement(EntityStatBlock, {
      entity: e,
      onMap: onMap
    }));
  }))));
...
function EntityStatBlock({
  ...
  const hpClass = hpPct <= 25 ? 'critical' : hpPct <= 50 ? 'low' : '';
  return /*#__PURE__*/React.createElement("div", {
    className: "entity-expanded"
  }, /*#__PURE__*/React.createElement("div", {
    className: "statblock-row"
  }, /*#__PURE__*/React.createElement("div", {
    className: "statblock-cell"
  }, /*#__PURE__*/React.createElement("div", {
    className: "statblock-label"
  }, "AC"), /*#__PURE__*/React.createElement("div", {
    className: "statblock-value mono"
  }, e.ac)), /*#__PURE__*/React.createElement("div", {
    className: "statblock-cell"
  }, /*#__PURE__*/React.createElement("div", {
    className: "statblock-label"
  }, "HP"), /*#__PURE__*/React.createElement("div", {
    className: "statblock-value mono"
  }, e.hp.current, /*#__PURE__*/React.createElement("span", {
    style: {
      color: 'var(--ink-mute)'
    }
  }, "/", e.hp.max))), /*#__PURE__*/React.createElement("div", {
    className: "statblock-cell"
  }, /*#__PURE__*/React.createElement("div", {
    className: "statblock-label"
  }, "Speed"), /*#__PURE__*/React.createElement("div", {
    className: "statblock-value mono"
  }, e.speed)), /*#__PURE__*/React.createElement("div", {
    className: "statblock-cell"
  }, /*#__PURE__*/React.createElement("div", {
    className: "statblock-label"
  }, "Init"), /*#__PURE__*/React.createElement("div", {
    className: "statblock-value mono"
  }, e.initBonus >= 0 ? `+${e.initBonus}` : e.initBonus))), /*#__PURE__*/React.createElement("div", {
    className: "statblock-hp-bar"
  }, /*#__PURE__*/React.createElement("div", {
    className: `statblock-hp-fill ${hpClass}`,
    style: {
      width: `${hpPct}%`
    }
  })), /*#__PURE__*/React.createElement("div", {
    className: "statblock-stats"
  }, ['str', 'dex', 'con', 'int', 'wis', 'cha'].map(s => /*#__PURE__*/React.createElement("div", {
    key: s,
    className: "statblock-stat"
  }, /*#__PURE__*/React.createElement("div", {
    className: "statblock-stat-label"
  }, s.toUpperCase()), /*#__PURE__*/React.createElement("div", {
    className: "statblock-stat-value mono"
  }, e.stats[s]), /*#__PURE__*/React.createElement("div", {
    className: "statblock-stat-mod mono"
  }, modFor(e.stats[s]) >= 0 ? `+${modFor(e.stats[s])}` : modFor(e.stats[s]))))), e.conditions.length > 0 && /*#__PURE__*/React.createElement("div", {
    className: "statblock-conditions"
  }, e.conditions.map(c => /*#__PURE__*/React.createElement("div", {
    key: c,
    className: "cond-chip active",
    style: {
      cursor: 'default'
    }
  }, c))), e.type === 'PC' && e.playerName && /*#__PURE__*/React.createElement("div", {
    className: "statblock-note"
  }, /*#__PURE__*/React.createElement("strong", null, "Player:"), " ", e.playerName), e.type === 'Monster' && e.abilities && /*#__PURE__*/React.createElement("div", {
    className: "statblock-note"
  }, /*#__PURE__*/React.createElement("strong", null, "Abilities:"), /*#__PURE__*/React.createElement("br", null), e.abilities), e.type === 'Monster' && e.playerDescription && /*#__PURE__*/React.createElement("div", {
    className: "statblock-note",
    style: {
  ...
  }, "Player-Visible:"), /*#__PURE__*/React.createElement("br", null), e.playerDescription), e.type === 'NPC' && (e.faction || e.role) && /*#__PURE__*/React.createElement("div", {
    className: "statblock-note"
  }, e.role && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("strong", null, "Role:"), " ", e.role, /*#__PURE__*/React.createElement("br", null)), e.faction && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("strong", null, "Faction:"), " ", e.faction)), e.notes && /*#__PURE__*/React.createElement("div", {
    className: "statblock-note"
  }, /*#__PURE__*/React.createElement("strong", null, "DM Notes:"), /*#__PURE__*/React.createElement("br", null), e.notes), onMap && /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 10,
      color: 'var(--ink-mute)',
      marginTop: 6,
...
function InitiativeTracker({
  ...
  const updateRoll = (entityId, newRoll) => {
    const entries = initiative.entries.map(e => e.entityId === entityId ? {
      ...e,
      roll: Number(newRoll) || 0
    } : e);
  ...
  return /*#__PURE__*/React.createElement(FloatPanel, {
    style: {
      right: 16,
      top: 80,
      width: 340
  ...
  }, /*#__PURE__*/React.createElement("span", null, "\u2694 Initiative \xB7 Round ", initiative.round), /*#__PURE__*/React.createElement("button", {
    className: "close-x",
    onClick: onClose
  }, "\xD7")), /*#__PURE__*/React.createElement("div", {
    className: "float-panel-body"
  }, mode === 'dm' && /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      gap: 6,
      marginBottom: 12
    ...
    className: "btn",
    onClick: advance,
    disabled: !initiative.entries.length
  }, "\u23ED Next Turn"), /*#__PURE__*/React.createElement("button", {
    className: "btn danger",
    onClick: clearInit,
    disabled: !initiative.entries.length
  }, "Clear")), /*#__PURE__*/React.createElement("div", {
    className: "init-list"
  }, initiative.entries.length === 0 ? /*#__PURE__*/React.createElement("div", {
    className: "empty-state"
  }, /*#__PURE__*/React.createElement("span", {
    className: "glyph"
  }, "\u2694"), "Initiative not yet rolled.") : initiative.entries.map((entry, idx) => {
    ...
    const monsterStatus = hpPctRaw <= 0 ? 'Down' : hpPctRaw < 30 ? 'Waning' : hpPctRaw <= 70 ? 'Rough' : 'Strong';
    return /*#__PURE__*/React.createElement("div", {
      key: entry.entityId,
      className: `init-entry ${idx === initiative.turn ? 'current' : ''}`
    }, mode === 'dm' ? /*#__PURE__*/React.createElement("input", {
      className: "mono",
      type: "number",
      value: entry.roll,
      ...
      className: "init-roll"
    }, entry.roll), /*#__PURE__*/React.createElement("div", {
      ...
      className: "init-name"
    }, e.name), showExactHp ? /*#__PURE__*/React.createElement("div", {
      className: "init-hp"
    }, e.hp.current, "/", e.hp.max) : /*#__PURE__*/React.createElement("div", {
      className: `init-status status-${monsterStatus.toLowerCase()}`
    }, monsterStatus), mode === 'dm' && /*#__PURE__*/React.createElement("button", {
      className: "btn sm ghost",
      onClick: () => removeEntry(entry.entityId),
      title: "Remove"
    }, "\xD7"));
  }))));
}
...
function MapManager({
  ...
  if (editing) {
    return /*#__PURE__*/React.createElement(FloatPanel, {
      ...
      className: "float-panel-header"
    }, /*#__PURE__*/React.createElement("span", null, "\u2316 ", state.maps[editing.id] ? 'Edit Map' : 'New Map'), /*#__PURE__*/React.createElement("button", {
      className: "close-x",
      ...
      value: "world"
    }, "World"), /*#__PURE__*/React.createElement("option", {
      value: "region"
    }, "Region"), /*#__PURE__*/React.createElement("option", {
      value: "city"
    }, "City"), /*#__PURE__*/React.createElement("option", {
      value: "dungeon"
    }, "Dungeon"), /*#__PURE__*/React.createElement("option", {
      value: "interior"
    }, "Interior"), /*#__PURE__*/React.createElement("option", {
      value: "encounter"
    }, "Encounter"))), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", null, "Parent Map"), /*#__PURE__*/React.createElement("select", {
      ...
      value: ""
    }, "\u2014 None \u2014"), maps.filter(m => m.id !== editing.id).map(m => /*#__PURE__*/React.createElement("option", {
      key: m.id,
      value: m.id
    }, m.name))))), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", null, "Map Image"), /*#__PURE__*/React.createElement("div", {
      style: {
        display: 'flex',
        gap: 8,
        alignItems: 'center'
      ...
      className: "btn",
      onClick: uploadImage
    }, "\uD83D\uDCC1 Upload Image"), editing.imageUrl && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("img", {
    ...
    }, "Embedded as base64 \u2014 stays in session.")), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", null, "Notes (DM only)"), /*#__PURE__*/React.createElement("textarea", {
      value: editing.notes,
      onChange: e => setEditing({
        ...editing,
        notes: e.target.value
    ...
    }, "Always dark ", /*#__PURE__*/React.createElement("span", {
      style: {
        color: 'var(--ink-mute)',
        fontWeight: 400,
        textTransform: 'none',
    ...
    }, "\u2014 overrides time of day; vision rules always apply"))), /*#__PURE__*/React.createElement("div", {
      style: {
        display: 'flex',
        gap: 8,
        justifyContent: 'flex-end'
  ...
  return /*#__PURE__*/React.createElement(FloatPanel, {
    ...
    className: "float-panel-header"
  }, /*#__PURE__*/React.createElement("span", null, "\u2316 Maps & Realms"), /*#__PURE__*/React.createElement("button", {
    className: "close-x",
    onClick: onClose
  }, "\xD7")), /*#__PURE__*/React.createElement("div", {
    className: "float-panel-body"
  ...
  }, "\uFF0B New Map"), /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      flexDirection: 'column',
      gap: 4
    ...
    return /*#__PURE__*/React.createElement("div", {
    ...
    }, m.imageUrl && /*#__PURE__*/React.createElement("img", {
      src: m.imageUrl,
      style: {
        width: 40,
        height: 40,
    ...
    }, m.type, parent ? ` · in ${parent}` : '')), /*#__PURE__*/React.createElement("button", {
      className: "btn sm",
      onClick: () => dispatch({
        type: 'MAP_SWITCH',
        id: m.id
      ...
      className: "btn sm ghost",
      onClick: () => setEditing(deepClone(m))
    }, "\u270E"), /*#__PURE__*/React.createElement("button", {
      className: "btn sm ghost",
...
function BestiaryMenu({
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "preset-menu bestiary-menu bestiary-modal",
    ...
    key: c,
    value: c
  }, c === 'All' ? 'All categories' : c)), custom.length > 0 && filterCategory !== 'Custom' && /*#__PURE__*/React.createElement("option", {
    value: "Custom"
  }, "Custom")), /*#__PURE__*/React.createElement("select", {
    className: "bestiary-select",
    value: filterType,
    onChange: e => setFilterType(e.target.value)
  }, typeOptions.map(t => /*#__PURE__*/React.createElement("option", {
    key: t,
    value: t
  }, t === 'All' ? 'All types' : t)))), /*#__PURE__*/React.createElement("div", {
    className: "bestiary-body"
  }, filtered.length === 0 ? /*#__PURE__*/React.createElement("div", {
    className: "bestiary-empty"
  }, "No matches. Try clearing filters.") : Object.entries(grouped).map(([cat, items]) => /*#__PURE__*/React.createElement("div", {
    key: cat,
    className: "bestiary-group"
  }, /*#__PURE__*/React.createElement("div", {
    className: "preset-menu-header"
  }, cat, " ", /*#__PURE__*/React.createElement("span", {
    style: {
      opacity: 0.5,
      fontWeight: 400
    }
  }, "\xB7 ", items.length)), items.map(p => /*#__PURE__*/React.createElement("div", {
    key: p.id,
    className: "preset-menu-item bestiary-item",
    onClick: () => onPick(p)
  }, /*#__PURE__*/React.createElement("div", {
    className: "preset-menu-swatch",
    style: {
      background: p.entity.color || '#888'
    }
  }), /*#__PURE__*/React.createElement("div", {
    style: {
      flex: 1,
    ...
    className: "preset-menu-name"
  }, p.name, p.cr && /*#__PURE__*/React.createElement("span", {
    className: "bestiary-cr"
  }, "CR ", p.cr)), /*#__PURE__*/React.createElement("div", {
    className: "preset-menu-type"
  }, p.entity.type, p.entity.role && /*#__PURE__*/React.createElement("span", {
    className: "bestiary-role"
  }, " \u2014 ", p.entity.role))), p._source === 'Custom' && /*#__PURE__*/React.createElement("button", {
    ...
    className: "preset-menu-footer"
  }, "Tap to create \xB7 drag a sidebar card \u2192 \"Save as preset\" to extend this list")));
}
...
function PresetsPanel({
  ...
  return /*#__PURE__*/React.createElement(FloatPanel, {
    ...
    className: "float-panel-header"
  }, /*#__PURE__*/React.createElement("span", null, "\u2748 Encounter Presets"), /*#__PURE__*/React.createElement("button", {
    className: "close-x",
    onClick: onClose
  }, "\xD7")), /*#__PURE__*/React.createElement("div", {
    className: "float-panel-body"
    ...
    className: "btn primary",
    onClick: savePreset
  }, "Save")), presets.length === 0 ? /*#__PURE__*/React.createElement("div", {
    className: "empty-state"
  }, /*#__PURE__*/React.createElement("span", {
    className: "glyph"
  }, "\u2748"), "No saved encounters yet.") : /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      flexDirection: 'column',
      gap: 4
    ...
    }, p.tokens.length, " tokens \xB7 ", map?.name || 'unknown map'), /*#__PURE__*/React.createElement("div", {
      style: {
        display: 'flex',
        gap: 4
      }
    ...
    }, "Overwrite"), /*#__PURE__*/React.createElement("button", {
      className: "btn sm danger",
...
function TokenDetailPanel({
  ...
  const labelState = entity.type === 'Label' && entity.hp.max > 0 ? hpPctRaw > 70 ? null : hpPctRaw > 50 ? 'Damaged' : hpPctRaw > 20 ? 'Derelict' : 'Ruins' : null;
  ...
  if (entity.type === 'Label') {
    ...
    }, "\u2726"), entity.name), /*#__PURE__*/React.createElement("button", {
      className: "close-x",
      onClick: onClose
    }, "\xD7")), /*#__PURE__*/React.createElement("div", {
      className: "float-panel-body"
    ...
    }, "Map label"), labelState && /*#__PURE__*/React.createElement("div", {
      className: `label-state-chip state-${labelState.toLowerCase()}`
    }, labelState), entity.playerDescription && /*#__PURE__*/React.createElement("div", {
      className: "statblock-note",
      style: {
        marginTop: 10
      }
    }, /*#__PURE__*/React.createElement("em", null, entity.playerDescription)), isDM && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
      style: {
        marginTop: 12,
        paddingTop: 10,
    ...
    }, "HP (drives state descriptor)"), /*#__PURE__*/React.createElement("div", {
      className: "form-row-2"
    }, /*#__PURE__*/React.createElement("input", {
      type: "number",
      value: entity.hp.current,
    ...
    }, token.visible ? '🕶 Hide' : '👁 Reveal'), /*#__PURE__*/React.createElement("button", {
      className: "btn sm danger",
      onClick: removeToken
    }, "\u2715 Remove")))));
  }
  return /*#__PURE__*/React.createElement(FloatPanel, {
  ...
  }), entity.name, isOwnPC && /*#__PURE__*/React.createElement("span", {
    className: "own-pc-badge"
  ...
  }, !isOpaqueForPlayer && /*#__PURE__*/React.createElement("div", {
    style: {
      textAlign: 'center',
      padding: 8,
      background: 'var(--bg-0)',
  ...
  }, entity.ac)), /*#__PURE__*/React.createElement("div", {
    style: {
      textAlign: 'center',
      padding: 8,
      background: 'var(--bg-0)',
  ...
  }, "HP"), isOpaqueForPlayer ? /*#__PURE__*/React.createElement("div", {
    className: `status-label status-${monsterStatus.toLowerCase()}`
  }, monsterStatus) : /*#__PURE__*/React.createElement("div", {
    className: "mono",
    style: {
      fontSize: 18,
      fontWeight: 600
    }
  }, entity.hp.current, /*#__PURE__*/React.createElement("span", {
    style: {
      color: 'var(--ink-mute)',
      fontSize: 12
    }
  }, "/", entity.hp.max))), !isOpaqueForPlayer && /*#__PURE__*/React.createElement("div", {
    style: {
      textAlign: 'center',
      padding: 8,
      background: 'var(--bg-0)',
  ...
  }, entity.speed))), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 11,
      color: 'var(--ink-dim)',
      marginBottom: 8
    }
  }, entity.type === 'PC' && `Level ${entity.level} ${entity.class || ''}${entity.playerName ? ` · ${entity.playerName}` : ''}`, entity.type === 'Monster' && isDM && `CR ${entity.cr}`, entity.type === 'NPC' && (entity.faction ? `${entity.role} · ${entity.faction}` : entity.role || 'NPC')), isOpaqueForPlayer && entity.playerDescription && /*#__PURE__*/React.createElement("div", {
    className: "statblock-note",
    style: {
      marginBottom: 10
    }
  }, entity.playerDescription), canEditHp && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("label", null, "Adjust HP ", isOwnPC && !isDM && /*#__PURE__*/React.createElement("span", {
    style: {
      color: 'var(--gold-dim)',
      fontWeight: 400,
      textTransform: 'none',
  ...
  }, "\u2014 your character")), /*#__PURE__*/React.createElement("div", {
    className: "hp-adjuster",
    style: {
      marginBottom: 10
    }
  }, /*#__PURE__*/React.createElement("button", {
    className: "btn danger",
    onClick: () => applyHp(-1)
  }, "\u2212 Damage"), /*#__PURE__*/React.createElement("input", {
    type: "number",
    value: hpDelta,
    onChange: e => setHpDelta(Math.abs(Number(e.target.value)) || 0)
  }), /*#__PURE__*/React.createElement("button", {
    className: "btn",
    onClick: () => applyHp(+1)
  ...
  }, /*#__PURE__*/React.createElement("label", null, "Conditions"), /*#__PURE__*/React.createElement("div", {
    className: "cond-grid"
  }, CONDITIONS.slice(0, 15).map(c => /*#__PURE__*/React.createElement("div", {
    key: c,
    className: `cond-chip ${entity.conditions.includes(c) ? 'active' : ''}`,
  ...
  }, c)))), isDM && entity.type === 'Monster' && entity.abilities && /*#__PURE__*/React.createElement("div", {
    style: {
      marginBottom: 10
    }
  }, /*#__PURE__*/React.createElement("label", null, "Abilities"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 12,
      padding: 8,
      background: 'var(--bg-0)',
  ...
  }, entity.abilities)), isDM && entity.notes && /*#__PURE__*/React.createElement("div", {
    style: {
      marginBottom: 10
    }
  }, /*#__PURE__*/React.createElement("label", null, "DM Notes"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 12,
      padding: 8,
      background: 'var(--bg-0)',
  ...
  }, entity.notes)), isDM && entity.type === 'PC' && /*#__PURE__*/React.createElement("div", {
    style: {
      marginBottom: 10
    }
  }, /*#__PURE__*/React.createElement("label", null, "Death Saves ", /*#__PURE__*/React.createElement("span", {
    style: {
      color: 'var(--ink-mute)',
      fontWeight: 400,
      textTransform: 'none',
  ...
  }, "\u2014 DM only")), /*#__PURE__*/React.createElement("div", {
    className: "death-saves"
  }, /*#__PURE__*/React.createElement("div", {
    className: "death-saves-row"
  }, /*#__PURE__*/React.createElement("span", {
    className: "death-saves-label good"
  }, "Successes"), /*#__PURE__*/React.createElement("div", {
    className: "death-pip-row"
  }, [1, 2, 3].map(n => {
    ...
    return /*#__PURE__*/React.createElement("button", {
      ...
      title: `Set successes to ${n}`
    }, "\u2713");
  }))), /*#__PURE__*/React.createElement("div", {
    className: "death-saves-row"
  }, /*#__PURE__*/React.createElement("span", {
    className: "death-saves-label bad"
    ...
    return /*#__PURE__*/React.createElement("button", {
      ...
      title: `Set failures to ${n}`
    }, "\u2717");
  }))), (entity.deathSaves?.successes > 0 || entity.deathSaves?.failures > 0) && /*#__PURE__*/React.createElement("button", {
    className: "btn sm ghost",
    style: {
      marginTop: 4
    },
    onClick: () => dispatch({
      type: 'DEATH_SAVE_CLEAR',
      id: entity.id
    })
  }, "Clear death saves"))), isDM && entity.type === 'Familiar' && state && /*#__PURE__*/React.createElement("div", {
    style: {
      marginBottom: 10
    }
  }, /*#__PURE__*/React.createElement("label", null, "Bonded with ", /*#__PURE__*/React.createElement("span", {
    style: {
      color: 'var(--ink-mute)',
      fontWeight: 400,
      textTransform: 'none',
  ...
  }, "\u2014 grants movement rights to whoever claims this PC")), /*#__PURE__*/React.createElement("select", {
    className: "mono",
    ...
    value: ""
  }, "\u2014 unbonded \u2014"), Object.values(state.entities).filter(ent => ent.type === 'PC').map(pc => {
    ...
    if (!pc) return /*#__PURE__*/React.createElement("div", {
      className: "settings-hint"
    }, "Bonded PC no longer exists.");
    if (!claim) return /*#__PURE__*/React.createElement("div", {
      className: "settings-hint"
    }, "Bond is dormant \u2014 no player has claimed ", pc.name, " yet.");
    return /*#__PURE__*/React.createElement("div", {
      className: "settings-hint"
    }, claim.playerName || 'A player', " controls this familiar.");
  })()), isDM && ['PC', 'Familiar', 'Monster', 'Neutral Beast', 'NPC'].includes(entity.type) && /*#__PURE__*/React.createElement("div", {
    style: {
      marginBottom: 10
    }
  }, /*#__PURE__*/React.createElement("label", null, "Vision ", /*#__PURE__*/React.createElement("span", {
    style: {
      color: 'var(--ink-mute)',
      fontWeight: 400,
      textTransform: 'none',
  ...
  }, "\u2014 used by the darkness system")), /*#__PURE__*/React.createElement("div", {
    className: "form-row-2"
  }, /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", {
    style: {
      fontSize: 9
    }
  }, "Darkvision (ft)"), /*#__PURE__*/React.createElement("input", {
    type: "number",
    min: "0",
    step: "5",
    value: entity.darkvision || 0,
  ...
  }, "Light Radius (ft)"), /*#__PURE__*/React.createElement("input", {
    type: "number",
    min: "0",
    step: "5",
    value: entity.lightRadius || 0,
  ...
  })))), isDM && ['PC', 'NPC', 'Monster', 'Neutral Beast', 'Familiar'].includes(entity.type) && /*#__PURE__*/React.createElement("div", {
    style: {
      marginBottom: 10
    }
  }, /*#__PURE__*/React.createElement("label", null, "Sickness ", /*#__PURE__*/React.createElement("span", {
    style: {
      color: 'var(--ink-mute)',
      fontWeight: 400,
      textTransform: 'none',
  ...
  }, "\u2014 creeping pallor on this player's view")), /*#__PURE__*/React.createElement("div", {
    className: "sickness-picker"
  }, [0, 1, 2, 3].map(lvl => /*#__PURE__*/React.createElement("button", {
    key: lvl,
    type: "button",
    ...
    className: "sickness-label"
  }, lvl === 0 ? 'Healthy' : SICKNESS_DESCRIPTORS[lvl]))))), isDM && /*#__PURE__*/React.createElement("div", {
    style: {
      marginBottom: 10
    }
  }, /*#__PURE__*/React.createElement("label", null, "Token Size ", /*#__PURE__*/React.createElement("span", {
    style: {
      color: 'var(--ink-mute)',
      fontWeight: 400,
      textTransform: 'none',
  ...
  }, "\u2014 scale on this map")), /*#__PURE__*/React.createElement("div", {
    className: "scale-row"
  }, /*#__PURE__*/React.createElement("button", {
    className: "btn sm",
    onClick: () => dispatch({
      type: 'TOKEN_SCALE',
      id: token.id,
      scale: Math.max(0.3, (token.scale || 1) - 0.1)
    })
  }, "\u2212"), /*#__PURE__*/React.createElement("input", {
    type: "range",
    min: "0.3",
    ...
    onClick: () => dispatch({
      type: 'TOKEN_SCALE',
      id: token.id,
      scale: Math.min(4, (token.scale || 1) + 0.1)
    })
  }, "+"), /*#__PURE__*/React.createElement("span", {
    className: "mono scale-value"
  }, ((token.scale || 1) * 100).toFixed(0), "%"), /*#__PURE__*/React.createElement("button", {
    className: "btn sm ghost",
    onClick: () => dispatch({
      type: 'TOKEN_SCALE',
      id: token.id,
  ...
  }, "Reset"))), isDM && /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      gap: 6,
      marginTop: 12,
    ...
    className: "btn",
    onClick: toggleVisibility
  }, token.visible ? '🕶 Hide from players' : '👁 Reveal to players'), onLongRest && (entity.type === 'PC' || entity.type === 'Familiar') && /*#__PURE__*/React.createElement("button", {
    className: "btn",
    onClick: () => onLongRest(entity.id),
    title: "Long rest this character only"
...
function TokenTooltip({
  ...
  const description = isDM ? ent.notes || ent.playerDescription || '' : ent.playerDescription || '';
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "token-tooltip-header"
  }, /*#__PURE__*/React.createElement("span", {
    className: "token-tooltip-name"
  }, ent.name), /*#__PURE__*/React.createElement("span", {
    className: `token-tooltip-type type-${TOKEN_SHAPE_CLASS[ent.type] || 'npc'}`
  }, ent.type)), ent.hp.max > 0 && (showHp ? /*#__PURE__*/React.createElement("div", {
    className: "token-tooltip-hp mono"
  }, "HP ", ent.hp.current, "/", ent.hp.max) : /*#__PURE__*/React.createElement("div", {
    className: `status-label status-${status.toLowerCase()}`
  }, status)), sicknessLabel && /*#__PURE__*/React.createElement("div", {
    className: `token-tooltip-sickness sick-level-${ent.sickness}`
  }, /*#__PURE__*/React.createElement("em", null, sicknessLabel.toLowerCase())), description && /*#__PURE__*/React.createElement("div", {
    className: "token-tooltip-desc"
  }, description), ent.conditions.length > 0 && /*#__PURE__*/React.createElement("div", {
    className: "token-tooltip-conds"
  }, ent.conditions.map(c => /*#__PURE__*/React.createElement("span", {
    key: c,
    className: "party-cond-pill",
    style: {
      background: CONDITION_COLORS[c] || '#555'
    }
  }, c))));
}
...
function SettingsModal({
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "float-panel-header"
  }, /*#__PURE__*/React.createElement("span", null, "\u2699 Settings"), /*#__PURE__*/React.createElement("button", {
    className: "close-x",
    onClick: onClose
  }, "\xD7")), /*#__PURE__*/React.createElement("div", {
    className: "float-panel-body"
    ...
    className: "theme-swatch dark"
  }), /*#__PURE__*/React.createElement("span", null, "Dark Sanctum"), /*#__PURE__*/React.createElement("span", {
    className: "theme-sub"
  }, "Navy \xB7 gilded")), /*#__PURE__*/React.createElement("button", {
    className: `theme-option ${settings.theme === 'light' ? 'active' : ''}`,
    onClick: () => onChange({
      theme: 'light'
    })
  }, /*#__PURE__*/React.createElement("span", {
    className: "theme-swatch light"
  }), /*#__PURE__*/React.createElement("span", null, "Warm Tavern"), /*#__PURE__*/React.createElement("span", {
    className: "theme-sub"
  }, "Parchment \xB7 oak")))), mode === 'dm' && /*#__PURE__*/React.createElement("div", {
    className: "settings-section"
  }, /*#__PURE__*/React.createElement("label", {
    className: "settings-label"
  }, "Map Scale ", /*#__PURE__*/React.createElement("span", {
    className: "settings-label-sub"
  }, "\u2014 how large the map feels relative to tokens")), /*#__PURE__*/React.createElement("div", {
    className: "scale-row"
  }, /*#__PURE__*/React.createElement("button", {
    className: "btn sm",
    onClick: () => onMapScaleChange(Math.max(0.3, (mapScale || 1) - 0.1))
  }, "\u2212"), /*#__PURE__*/React.createElement("input", {
    type: "range",
    min: "0.3",
    max: "3",
    ...
    className: "btn sm",
    onClick: () => onMapScaleChange(Math.min(3, (mapScale || 1) + 0.1))
  }, "+"), /*#__PURE__*/React.createElement("span", {
    className: "mono scale-value"
  }, ((mapScale || 1) * 100).toFixed(0), "%"), /*#__PURE__*/React.createElement("button", {
    className: "btn sm ghost",
    onClick: () => onMapScaleChange(1)
  }, "Reset")), /*#__PURE__*/React.createElement("div", {
    className: "settings-hint"
  }, "Scales the entire map rendering uniformly. Pan/zoom still works on top.")), /*#__PURE__*/React.createElement("div", {
    className: "settings-section"
  }, /*#__PURE__*/React.createElement("div", {
    className: "settings-hint",
    style: {
...
function useDraggable(ref) {
  ...
  useEffect(() => {
    ...
    const onPointerMove = e => {
      ...
      setOffset({
        dx: newLeft - originalLeft,
        dy: newTop - originalTop
      });
    };
    const onPointerUp = e => {
...
function EditMySheetModal({
  ...
  if (!entity) {
    return /*#__PURE__*/React.createElement("div", {
      ...
      className: "float-panel-header"
    }, /*#__PURE__*/React.createElement("span", null, "\u25C8 Edit My Sheet"), /*#__PURE__*/React.createElement("button", {
      className: "close-x",
      onClick: onClose
    }, "\xD7")), /*#__PURE__*/React.createElement("div", {
      className: "float-panel-body"
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }), "\u25C8 ", entity.name, " \u2014 Your Sheet"), /*#__PURE__*/React.createElement("button", {
    className: "close-x",
    onClick: onClose
  }, "\xD7")), /*#__PURE__*/React.createElement("div", {
    className: "float-panel-body"
  }, myIds.length > 1 && /*#__PURE__*/React.createElement("div", {
    ...
    }), e.name, id === claim.pc ? /*#__PURE__*/React.createElement("span", {
      className: "own-pc-badge",
      style: {
        marginLeft: 4
      }
    }, "PC") : /*#__PURE__*/React.createElement("span", {
      className: "familiar-badge"
    }, "FAM"));
  })), /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
    ...
    type: "button",
    onClick: uploadImage
  }, "\u21E7 Upload portrait"), entity.imageUrl && /*#__PURE__*/React.createElement("button", {
    className: "btn sm ghost",
    type: "button",
    onClick: () => setField({
      imageUrl: ''
    })
  }, "Remove image"), /*#__PURE__*/React.createElement("input", {
    type: "color",
    value: entity.color,
    onChange: e => setField({
      color: e.target.value
  ...
  }))), entity.type === 'PC' && sicknessLabel && /*#__PURE__*/React.createElement("div", {
    className: `sickness-note sick-level-${entity.sickness || 0}`
  }, /*#__PURE__*/React.createElement("span", {
    className: "sickness-glyph"
  }, "\u274B"), /*#__PURE__*/React.createElement("span", null, /*#__PURE__*/React.createElement("em", null, "You feel"), " ", /*#__PURE__*/React.createElement("strong", null, sicknessLabel.toLowerCase()), ".")), /*#__PURE__*/React.createElement("div", {
    className: "sheet-stats"
  }, /*#__PURE__*/React.createElement("div", {
    className: "sheet-stat"
  }, /*#__PURE__*/React.createElement("span", null, "AC"), /*#__PURE__*/React.createElement(LiveNumberInput, {
    className: "sheet-stat-input mono",
    value: entity.ac,
    onCommit: v => setField({
      ac: v
    }),
    ...
    className: "sheet-stat"
  }, /*#__PURE__*/React.createElement("span", null, "Speed"), /*#__PURE__*/React.createElement(LiveNumberInput, {
    className: "sheet-stat-input mono",
    value: entity.speed,
    onCommit: v => setField({
      speed: v
    ...
    className: "sheet-stat"
  }, /*#__PURE__*/React.createElement("span", null, "Init"), /*#__PURE__*/React.createElement(LiveNumberInput, {
    className: "sheet-stat-input mono",
    value: entity.initBonus,
    onCommit: v => setField({
      initBonus: v
    ...
    className: "sheet-stat"
  }, /*#__PURE__*/React.createElement("span", null, "Passive"), /*#__PURE__*/React.createElement(LiveNumberInput, {
    className: "sheet-stat-input mono",
    value: entity.passivePerception,
    onCommit: v => setField({
      passivePerception: v
    ...
    min: 0,
    max: 40
  }))), /*#__PURE__*/React.createElement("label", null, "HP"), /*#__PURE__*/React.createElement("div", {
    className: "form-row-2",
    style: {
      marginBottom: 8
    }
  ...
  }, "Current"), /*#__PURE__*/React.createElement(LiveNumberInput, {
    value: entity.hp.current,
    onCommit: v => setField({
      hp: {
        current: v,
  ...
  }, "Max"), /*#__PURE__*/React.createElement(LiveNumberInput, {
    value: entity.hp.max,
    onCommit: v => setField({
      hp: {
    ...
    min: 0,
    max: 10000
  }))), /*#__PURE__*/React.createElement("label", null, "Quick Adjust"), /*#__PURE__*/React.createElement("div", {
    className: "hp-adjuster",
    style: {
      marginBottom: 10
    }
    ...
    className: "btn",
    onClick: () => applyHp(+1)
  }, "+ Heal")), ['PC', 'Familiar'].includes(entity.type) && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("label", null, "Vision ", /*#__PURE__*/React.createElement("span", {
    style: {
      color: 'var(--ink-mute)',
      fontWeight: 400,
      textTransform: 'none',
  ...
  }, "\u2014 feet; DM may override")), /*#__PURE__*/React.createElement("div", {
    className: "form-row-2",
    style: {
      marginBottom: 10
    }
  ...
  }, "Darkvision"), /*#__PURE__*/React.createElement(LiveNumberInput, {
    value: entity.darkvision || 0,
    onCommit: v => setField({
      darkvision: v
    }),
  ...
  }, "Light Radius"), /*#__PURE__*/React.createElement(LiveNumberInput, {
    value: entity.lightRadius || 0,
    onCommit: v => setField({
      lightRadius: v
    }),
    ...
    max: 600,
    step: 5
  })))), entity.type === 'PC' && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("label", {
    onClick: () => setExpandedSection(s => s === 'stats' ? '' : 'stats'),
    style: {
      cursor: 'pointer',
      userSelect: 'none'
    }
  }, "Ability Scores ", expandedSection === 'stats' ? '▾' : '▸'), expandedSection === 'stats' && /*#__PURE__*/React.createElement("div", {
    className: "form-row-6",
    style: {
      marginBottom: 10
    }
    ...
    key: s,
    className: "stat-box"
  }, /*#__PURE__*/React.createElement("label", null, s.toUpperCase()), /*#__PURE__*/React.createElement(LiveNumberInput, {
    value: entity.stats[s],
    onCommit: v => setField({
      stats: {
        [s]: v
  ...
  }, modFor(entity.stats[s]) >= 0 ? `+${modFor(entity.stats[s])}` : modFor(entity.stats[s])))))), entity.type === 'PC' && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("label", {
    onClick: () => setExpandedSection(s => s === 'identity' ? '' : 'identity'),
    style: {
      cursor: 'pointer',
      userSelect: 'none'
    }
  }, "Identity ", expandedSection === 'identity' ? '▾' : '▸'), expandedSection === 'identity' && /*#__PURE__*/React.createElement("div", {
    style: {
      marginBottom: 10
    }
  }, /*#__PURE__*/React.createElement("div", {
  ...
  }, "Name"), /*#__PURE__*/React.createElement(LiveInput, {
    value: entity.name,
    onCommit: v => setField({
      name: v
    })
  ...
  }, "Class"), /*#__PURE__*/React.createElement(LiveInput, {
    value: entity.class || '',
    onCommit: v => setField({
      class: v
    })
  ...
  }, "Level"), /*#__PURE__*/React.createElement(LiveNumberInput, {
    value: entity.level,
    onCommit: v => setField({
      level: Math.max(1, v)
    }),
  ...
  }, "Player Name"), /*#__PURE__*/React.createElement(LiveInput, {
    value: entity.playerName || '',
    onCommit: v => setField({
      playerName: v
    })
  }))))), /*#__PURE__*/React.createElement("label", null, "Conditions ", /*#__PURE__*/React.createElement("span", {
    style: {
      color: 'var(--ink-mute)',
      fontWeight: 400,
      textTransform: 'none',
  ...
  }, "\u2014 click to toggle")), /*#__PURE__*/React.createElement("div", {
    className: "cond-grid"
  }, CONDITIONS.slice(0, 15).map(c => /*#__PURE__*/React.createElement("div", {
    key: c,
    className: `cond-chip ${entity.conditions.includes(c) ? 'active' : ''}`,
  ...
  }, "Notes / Description"), /*#__PURE__*/React.createElement(LiveTextarea, {
    value: entity.playerDescription || '',
    onCommit: v => setField({
      playerDescription: v
    }),
...
function PlayerOnboardingGate({
  ...
  const availablePCs = Object.values(state.entities).filter(e => e.type === 'PC' && !allClaimedIds.has(e.id)).filter(e => !search || e.name.toLowerCase().includes(search.toLowerCase()));
  const pickPC = ent => {
    playerActionSender({
      type: 'claim_pc',
      payload: {
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, "Step into the realm"), /*#__PURE__*/React.createElement("div", {
    className: "onboarding-subtitle"
  }, "Welcome, ", playerName || 'traveler', ". Choose your presence at the table."), /*#__PURE__*/React.createElement("div", {
    className: "onboarding-section"
  }, /*#__PURE__*/React.createElement("div", {
    className: "onboarding-section-title"
  }, "Existing Characters"), availablePCs.length === 0 ? /*#__PURE__*/React.createElement("div", {
    className: "empty-state",
    style: {
      padding: '20px'
    }
    ...
    className: "glyph"
  }, "\u2694"), "No unclaimed characters. Ask your DM to create one for you, or proceed as a spectator.") : /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("input", {
  ...
  }, availablePCs.map(e => /*#__PURE__*/React.createElement("div", {
    key: e.id,
  ...
  }, e.imageUrl ? /*#__PURE__*/React.createElement("img", {
    src: e.imageUrl,
    alt: "",
    style: {
      width: '100%',
  ...
  }) : (e.name[0] || '?').toUpperCase()), /*#__PURE__*/React.createElement("div", {
    style: {
      flex: 1
    }
  }, /*#__PURE__*/React.createElement("div", {
  ...
  }, "Level ", e.level, " ", e.class, " \xB7 ", e.hp.max, " HP \xB7 AC ", e.ac)), /*#__PURE__*/React.createElement("button", {
    ...
    className: "btn",
    onClick: onRequestNewPC
  }, "\uFF0B Request a new character"), /*#__PURE__*/React.createElement("button", {
    className: "btn ghost",
    onClick: pickSpectator
  }, "\uD83D\uDC41 Join as spectator")), /*#__PURE__*/React.createElement("div", {
    className: "settings-hint",
    style: {
      textAlign: 'center',
      marginTop: 16
...
const HAZARD_KINDS = [{
  ...
  label: 'Fire',
  glyph: '🔥',
  swatch: 'rgba(230,80,40,0.6)'
}, {
  key: 'flood',
  label: 'Flood',
  glyph: '🌊',
  swatch: 'rgba(60,120,200,0.6)'
}, {
  key: 'cold',
  label: 'Cold',
  glyph: '❄',
  swatch: 'rgba(200,230,245,0.7)'
}, {
  key: 'acid',
  label: 'Acid',
  glyph: '☣',
  swatch: 'rgba(110,180,70,0.6)'
}, {
  key: 'fog',
  label: 'Fog',
  glyph: '☁',
  swatch: 'rgba(180,180,190,0.65)'
}, {
  key: 'difficult',
  label: 'Difficult',
  glyph: '⟁',
  swatch: 'rgba(160,110,50,0.55)'
}];
function HazardsPanel({
  ...
  placingHazard,
  setPlacingHazard,
  hazardVisibleDefault,
  setHazardVisibleDefault
}) {
  ...
  return /*#__PURE__*/React.createElement(FloatPanel, {
    ...
    className: "float-panel-header"
  }, /*#__PURE__*/React.createElement("span", null, "\u26A0 Hazards"), /*#__PURE__*/React.createElement("button", {
    className: "close-x",
    onClick: onClose
  }, "\xD7")), /*#__PURE__*/React.createElement("div", {
    className: "float-panel-body"
    ...
    className: "settings-label-sub"
  }, "\u2014 click-drag on map to paint")), /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'grid',
  ...
  }, HAZARD_KINDS.map(k => /*#__PURE__*/React.createElement("button", {
    key: k.key,
  ...
  }), k.glyph, " ", k.label))), /*#__PURE__*/React.createElement("label", {
    style: {
      display: 'flex',
      alignItems: 'center',
      gap: 6,
    ...
    type: "checkbox",
    checked: hazardVisibleDefault,
    onChange: e => setHazardVisibleDefault(e.target.checked)
  }), /*#__PURE__*/React.createElement("span", null, "New hazards visible to players")), /*#__PURE__*/React.createElement("div", {
    className: "settings-hint",
    style: {
      marginBottom: 10
    }
  }, "Uncheck to paint hidden hazards (traps). Hidden hazards only appear on the DM screen."), /*#__PURE__*/React.createElement("label", {
    className: "settings-label"
  }, "On this map ", /*#__PURE__*/React.createElement("span", {
    className: "settings-label-sub"
  }, "\u2014 ", list.length, " hazard(s)")), /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      flexDirection: 'column',
      gap: 4,
  ...
  }, list.map(h => /*#__PURE__*/React.createElement("div", {
    key: h.id,
    style: {
      display: 'flex',
      alignItems: 'center',
  ...
  }, HAZARD_KINDS.find(k => k.key === h.hazardKind)?.glyph || '?', " ", h.hazardKind, h.visible === false && /*#__PURE__*/React.createElement("span", {
    style: {
      color: 'var(--ink-mute)',
      marginLeft: 4
    }
  ...
  }, h.visible === false ? '👁' : '🕶'), /*#__PURE__*/React.createElement("button", {
  ...
  }, "No hazards painted yet.")), /*#__PURE__*/React.createElement("button", {
    className: "btn sm danger",
    disabled: !list.length,
    onClick: () => {
      if (confirm('Clear all hazards on this map?')) {
        dispatch({
          type: 'HAZARD_CLEAR_MAP',
          mapId: currentMapId
        });
...
function ToolsMenu({
  ...
  let activeLabel = '';
  if (measureMode === 'line') activeLabel = '· Measure';else if (measureMode === 'radius') activeLabel = '· Radius';else if (measureMode === 'tokenToToken') activeLabel = '· T→T';else if (placingReminder) activeLabel = '· Reminder';else if (isDM && placingBlock) activeLabel = '· Block';else if (isDM && placingFreeBlock) activeLabel = '· Block';else if (isDM && placingCircleBlock) activeLabel = '· Block';else if (isDM && erasingBlock) activeLabel = '· Eraser';
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: `btn ${open || isActive ? 'active' : ''}`,
    onClick: () => setOpen(!open),
    title: "Tools \u2014 measure, draw, shapes, dice, sounds"
  }, "\uD83E\uDDF0 Tools", activeLabel ? /*#__PURE__*/React.createElement("span", {
    className: "tools-active-suffix"
  }, " ", activeLabel) : ''), open && /*#__PURE__*/React.createElement("div", {
    className: "tools-menu-pop"
  }, /*#__PURE__*/React.createElement("div", {
    className: "tools-section"
  }, /*#__PURE__*/React.createElement("div", {
    className: "tools-section-title"
  }, "Measure"), /*#__PURE__*/React.createElement("button", {
    className: `tools-item ${measureMode === 'line' ? 'active' : ''}`,
    onClick: choose(() => setMeasureMode('line'))
  }, "\uD83D\uDCCF Line ", /*#__PURE__*/React.createElement("span", {
    className: "tools-hint"
  }, "click-drag")), /*#__PURE__*/React.createElement("button", {
    className: `tools-item ${measureMode === 'radius' ? 'active' : ''}`,
    onClick: choose(() => setMeasureMode('radius'))
  }, "\u25CE Radius ", /*#__PURE__*/React.createElement("span", {
    className: "tools-hint"
  }, "drag from center")), /*#__PURE__*/React.createElement("button", {
    className: `tools-item ${measureMode === 'tokenToToken' ? 'active' : ''}`,
    onClick: choose(() => setMeasureMode('tokenToToken'))
  }, "\u2934 Token \u2192 Token ", /*#__PURE__*/React.createElement("span", {
    className: "tools-hint"
  }, "click two tokens"))), /*#__PURE__*/React.createElement("div", {
    className: "tools-section"
  }, /*#__PURE__*/React.createElement("div", {
    className: "tools-section-title"
  }, "Draw"), /*#__PURE__*/React.createElement("button", {
    className: `tools-item ${showDraw ? 'active' : ''}`,
    onClick: () => {
      setShowDraw(true);
      setOpen(false);
    }
  }, "\u2712 Drawing palette ", /*#__PURE__*/React.createElement("span", {
    className: "tools-hint"
  }, "free / line / circle"))), isDM && /*#__PURE__*/React.createElement("div", {
    className: "tools-section"
  }, /*#__PURE__*/React.createElement("div", {
    className: "tools-section-title"
  }, "Shapes & Areas ", /*#__PURE__*/React.createElement("span", {
    className: "tools-section-sub"
  }, "DM")), /*#__PURE__*/React.createElement("button", {
    className: `tools-item ${placingBlock ? 'active' : ''}`,
    onClick: choose(() => setPlacingBlock(true))
  }, "\u25FC Block \u2014 Rect ", /*#__PURE__*/React.createElement("span", {
    className: "tools-hint"
  }, "click-drag")), /*#__PURE__*/React.createElement("button", {
    className: `tools-item ${placingFreeBlock ? 'active' : ''}`,
    onClick: choose(() => setPlacingFreeBlock(true))
  }, "\u270E Block \u2014 Freeform ", /*#__PURE__*/React.createElement("span", {
    className: "tools-hint"
  }, "drag a polygon")), /*#__PURE__*/React.createElement("button", {
    className: `tools-item ${placingCircleBlock ? 'active' : ''}`,
    onClick: choose(() => setPlacingCircleBlock(true))
  }, "\u2B24 Block \u2014 Circle ", /*#__PURE__*/React.createElement("span", {
    className: "tools-hint"
  }, "drag from center")), /*#__PURE__*/React.createElement("button", {
    className: `tools-item ${showHazards ? 'active' : ''}`,
    onClick: () => {
      setShowHazards(true);
      setOpen(false);
    }
  }, "\u26A0 Hazards palette ", /*#__PURE__*/React.createElement("span", {
    className: "tools-hint"
  }, "fire / flood / cold / acid / fog / difficult")), /*#__PURE__*/React.createElement("button", {
    className: `tools-item danger ${erasingBlock ? 'active' : ''}`,
    onClick: choose(() => setErasingBlock(true))
  }, "\u2715 Cut / Eraser ", /*#__PURE__*/React.createElement("span", {
    className: "tools-hint"
  }, "draw to subtract"))), isDM && /*#__PURE__*/React.createElement("div", {
    className: "tools-section"
  }, /*#__PURE__*/React.createElement("div", {
    className: "tools-section-title"
  }, "Encounter ", /*#__PURE__*/React.createElement("span", {
    className: "tools-section-sub"
  }, "DM")), /*#__PURE__*/React.createElement("button", {
    className: `tools-item ${showGroups ? 'active' : ''}`,
    onClick: () => {
      setShowGroups(true);
      setOpen(false);
    }
  }, "\u22F1 Token groups ", /*#__PURE__*/React.createElement("span", {
    className: "tools-hint"
  }, "reveal / hide clusters at once"))), /*#__PURE__*/React.createElement("div", {
    className: "tools-section"
  }, /*#__PURE__*/React.createElement("div", {
    className: "tools-section-title"
  }, "Other"), /*#__PURE__*/React.createElement("button", {
    className: `tools-item ${placingReminder ? 'active' : ''}`,
    onClick: choose(() => setPlacingReminder(true))
  }, "\u25C6 Reminder ", /*#__PURE__*/React.createElement("span", {
    className: "tools-hint"
  }, "private pin")), /*#__PURE__*/React.createElement("button", {
    className: `tools-item ${showDice ? 'active' : ''}`,
    onClick: () => {
      setShowDice(true);
      setOpen(false);
    }
    ...
    className: "tools-hint"
  }, "d4 \u2014 d20 for the table")), isDM && /*#__PURE__*/React.createElement("button", {
    className: `tools-item ${showSounds ? 'active' : ''}`,
    onClick: () => {
      setShowSounds(true);
      setOpen(false);
    }
  }, "\uD83D\uDD0A Soundboard ", /*#__PURE__*/React.createElement("span", {
    className: "tools-hint"
  }, "play audio for everyone"))), isActive && /*#__PURE__*/React.createElement("div", {
    className: "tools-section"
  }, /*#__PURE__*/React.createElement("button", {
    className: "tools-item ghost",
    onClick: () => {
      clearAllModes();
      setOpen(false);
    }
...
function GroupsPanel({
  ...
  selectedTokenIds,
  onTokenReveal,
  // (tokenId, visible) — reuses existing TOKEN_VISIBILITY plumbing if needed
  ...
  const groupsOnMap = useMemo(() => Object.values(groupsById).filter(g => g.mapId === currentMapId).sort((a, b) => (a.createdTs || 0) - (b.createdTs || 0)), [groupsById, currentMapId]);
  ...
  const visibilitySummary = g => {
    const members = (g.memberIds || []).map(tid => state.tokens?.[tid]).filter(Boolean);
  ...
  return /*#__PURE__*/React.createElement(FloatPanel, {
    ...
    className: "float-panel-header"
  }, /*#__PURE__*/React.createElement("span", null, "\u22F1 Groups"), /*#__PURE__*/React.createElement("button", {
    className: "close-x",
    onClick: onClose
  }, "\xD7")), /*#__PURE__*/React.createElement("div", {
    className: "float-panel-body"
  ...
  }, "Cluster tokens for faster encounter control \u2014 hide or reveal a whole ambush at once."), /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      gap: 6,
      marginBottom: 10
    ...
    title: selectedOnCurrentMap.length === 0 ? 'Shift-click tokens on the map to select them first' : `Create a group from ${selectedOnCurrentMap.length} selected token${selectedOnCurrentMap.length === 1 ? '' : 's'}`
  }, "\uFF0B From selection (", selectedOnCurrentMap.length, ")"), /*#__PURE__*/React.createElement("button", {
    className: "btn sm ghost",
    onClick: createEmpty,
    title: "Create an empty group and add members later"
  }, "\uFF0B Empty")), /*#__PURE__*/React.createElement("label", {
    className: "settings-label"
  }, "Groups on this map ", /*#__PURE__*/React.createElement("span", {
    className: "settings-label-sub"
  }, "\u2014 ", groupsOnMap.length)), groupsOnMap.length === 0 && /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 11,
      color: 'var(--ink-dim)',
      fontStyle: 'italic',
  ...
  }, "No groups yet. Select tokens on the map and click \"From selection\"."), /*#__PURE__*/React.createElement("div", {
    ...
    return /*#__PURE__*/React.createElement("div", {
      key: g.id,
      className: `group-row ${isOpen ? 'open' : ''}`
    }, /*#__PURE__*/React.createElement("div", {
      className: "group-row-head"
      ...
      className: "group-row-label"
    }, g.name), /*#__PURE__*/React.createElement("span", {
      className: `group-row-summary ${summary.mixed ? 'mixed' : ''}`
    }, summary.label)), /*#__PURE__*/React.createElement("div", {
      ...
      title: "Delete group (tokens are preserved)"
    }, "\u2715"))), isOpen && /*#__PURE__*/React.createElement("div", {
      className: "group-row-body"
    }, /*#__PURE__*/React.createElement("div", {
      className: "group-row-members"
    }, (g.memberIds || []).length === 0 && /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: 10,
        fontStyle: 'italic',
        color: 'var(--ink-dim)',
    ...
    }, "No members. Select tokens on the map and click \"Add selection\" below."), (g.memberIds || []).map(tid => {
      const t = state.tokens?.[tid];
      const ent = t ? state.entities?.[t.entityId] : null;
      if (!t || !ent) return null;
      return /*#__PURE__*/React.createElement("div", {
        key: tid,
        className: "group-row-member"
      }, /*#__PURE__*/React.createElement("div", {
        className: "entity-swatch",
        style: {
          background: ent.color || 'var(--gold)',
          width: 10,
          height: 10
        }
      }), /*#__PURE__*/React.createElement("span", {
        className: "group-row-member-name"
      }, ent.name || 'Unnamed'), /*#__PURE__*/React.createElement("span", {
        className: `group-row-member-vis ${t.visible ? 'visible' : 'hidden'}`
      }, t.visible ? 'visible' : 'hidden'), /*#__PURE__*/React.createElement("button", {
        className: "btn sm ghost danger",
        onClick: () => removeMember(g.id, tid),
        title: "Remove from group"
      }, "\u2212"));
    })), selectedOnCurrentMap.length > 0 && /*#__PURE__*/React.createElement("button", {
      className: "btn sm",
...
function SoundboardPanel({
  ...
  const handleUpload = async e => {
    ...
    try {
      for (const file of files) {
        ...
        await idbSet(IDB_STORES.sounds, id, {
          id,
          name,
          dataUrl,
          ts: Date.now()
  ...
  return /*#__PURE__*/React.createElement(FloatPanel, {
    ...
    className: "float-panel-header"
  }, /*#__PURE__*/React.createElement("span", null, "\uD83D\uDD0A Soundboard"), /*#__PURE__*/React.createElement("button", {
    className: "close-x",
    onClick: onClose
  }, "\xD7")), /*#__PURE__*/React.createElement("div", {
    className: "float-panel-body"
  }, !audioReady && /*#__PURE__*/React.createElement("div", {
    className: "settings-hint",
    style: {
  ...
  }, "Click anywhere to enable audio playback (browser policy)."), /*#__PURE__*/React.createElement("label", {
    className: "settings-label"
  }, "Upload ", /*#__PURE__*/React.createElement("span", {
    className: "settings-label-sub"
  }, "\u2014 mp3, ogg, wav")), /*#__PURE__*/React.createElement("input", {
    ref: fileRef,
    type: "file",
    accept: "audio/*",
    multiple: true,
    ...
    className: "settings-label-sub"
  }, "\u2014 ", list.length, " sound", list.length === 1 ? '' : 's')), /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      flexDirection: 'column',
      gap: 4,
  ...
  }, "No sounds yet. Upload audio files to build your soundboard."), list.map(s => /*#__PURE__*/React.createElement("div", {
    key: s.id,
    style: {
      display: 'flex',
      alignItems: 'center',
    ...
    className: "btn sm",
    onClick: () => onPlay(s.id),
    title: "Play for the table"
  }, "\u25B6"), /*#__PURE__*/React.createElement("button", {
    className: "btn sm ghost",
    onClick: () => onStop(s.id),
    title: "Stop"
  }, "\u25A0"), isDM && /*#__PURE__*/React.createElement("button", {
    className: "btn sm ghost danger",
...
function DiceTray({
  ...
  const expression = ALLOWED_DIE_SIDES.filter(s => counts[s] > 0).map(s => `${counts[s]}d${s}`).join(' + ') || '(pick dice)';
  ...
  const quickD20 = () => {
    const entry = rollDiceMixed({
      20: 1
    }, myPeerId || (isDM ? 'dm' : 'player'), myName);
    onRoll(entry);
  };
  const fmtTime = ts => {
    const d = new Date(ts);
  ...
  const renderEntry = e => {
    ...
    const expr = e.expression || groups.map(g => `${g.results.length}d${g.die}`).join(' + ');
    return /*#__PURE__*/React.createElement("div", {
      key: e.id,
      className: `dice-log-entry ${isMine ? 'mine' : ''} ${isCrit20 ? 'crit' : ''} ${isCrit1 ? 'fail' : ''}`
    }, /*#__PURE__*/React.createElement("div", {
      className: "dice-log-head"
    }, /*#__PURE__*/React.createElement("span", {
      className: "dice-log-who"
    }, e.peerName), /*#__PURE__*/React.createElement("span", {
      className: "dice-log-when"
    }, fmtTime(e.ts))), /*#__PURE__*/React.createElement("div", {
      className: "dice-log-roll"
    }, /*#__PURE__*/React.createElement("span", {
      className: "dice-log-spec"
    }, expr), /*#__PURE__*/React.createElement("span", {
      className: "dice-log-detail"
    }, "= ", /*#__PURE__*/React.createElement("strong", null, e.total))), groups.some(g => g.results.length > 1 || groups.length > 1) && /*#__PURE__*/React.createElement("div", {
      className: "dice-log-breakdown"
    }, groups.map((g, i) => /*#__PURE__*/React.createElement("div", {
      key: i,
      className: "dice-log-breakdown-row"
    }, /*#__PURE__*/React.createElement("span", {
      className: "dice-log-breakdown-die"
    }, "d", g.die, ":"), /*#__PURE__*/React.createElement("span", {
      className: "dice-log-breakdown-vals"
    }, g.results.join(', '))))));
  };
  return /*#__PURE__*/React.createElement(FloatPanel, {
    style: {
    ...
    className: "float-panel-header"
  }, /*#__PURE__*/React.createElement("span", null, "\uD83C\uDFB2 Dice"), /*#__PURE__*/React.createElement("button", {
    className: "close-x",
    onClick: onClose
  }, "\xD7")), /*#__PURE__*/React.createElement("div", {
    className: "float-panel-body"
    ...
    className: "settings-label"
  }, "Build your roll", totalDice > 0 && /*#__PURE__*/React.createElement("button", {
    className: "btn sm ghost",
    style: {
      float: 'right',
      marginTop: -4
    ...
    className: "dice-steppers"
  }, ALLOWED_DIE_SIDES.map(s => /*#__PURE__*/React.createElement("div", {
    key: s,
    className: "dice-stepper"
  }, /*#__PURE__*/React.createElement("span", {
    className: "dice-stepper-label"
  }, "d", s), /*#__PURE__*/React.createElement("button", {
    className: "dice-stepper-btn",
    onClick: () => bump(s, -1),
    disabled: counts[s] <= 0,
    "aria-label": `Remove a d${s}`
  }, "\u2212"), /*#__PURE__*/React.createElement("input", {
    className: "dice-stepper-input",
    type: "number",
    min: "0",
    max: "100",
    value: counts[s],
    ...
    className: "dice-expression-label"
  }, "Expression"), /*#__PURE__*/React.createElement("span", {
    className: "dice-expression-value"
  ...
  }, "\uD83C\uDFB2 Roll"), /*#__PURE__*/React.createElement("button", {
    className: "btn sm",
    onClick: quickD20,
    title: "Quick d20 without changing the tray"
  }, "d20")), /*#__PURE__*/React.createElement("label", {
    className: "settings-label"
  }, "Recent ", /*#__PURE__*/React.createElement("span", {
    className: "settings-label-sub"
  }, "\u2014 ", log.length), isDM && log.length > 0 && /*#__PURE__*/React.createElement("button", {
    className: "btn sm ghost danger",
    style: {
      float: 'right',
      marginTop: -4
  ...
  }, "No rolls yet. Pick some dice above and tap Roll."), log.map(renderEntry))));
}
...
function DrawingPanel({
  ...
  return /*#__PURE__*/React.createElement(FloatPanel, {
    ...
    className: "float-panel-header"
  }, /*#__PURE__*/React.createElement("span", null, "\u2712 Draw"), /*#__PURE__*/React.createElement("button", {
    className: "close-x",
    onClick: onClose
  }, "\xD7")), /*#__PURE__*/React.createElement("div", {
    className: "float-panel-body"
    ...
    className: `btn sm ${drawMode === 'free' ? 'active' : ''}`,
    onClick: () => setDrawMode(drawMode === 'free' ? null : 'free')
  }, "\u2712 Free"), /*#__PURE__*/React.createElement("button", {
    className: `btn sm ${drawMode === 'line' ? 'active' : ''}`,
    onClick: () => setDrawMode(drawMode === 'line' ? null : 'line')
  }, "\u2571 Line"), /*#__PURE__*/React.createElement("button", {
    className: `btn sm ${drawMode === 'circle' ? 'active' : ''}`,
    ...
    style: {
      background: c,
      outline: drawColor === c ? '2px solid var(--gold-bright)' : '1px solid var(--border-soft)'
    },
    ...
    className: "settings-label-sub"
  }, "(", drawWidth, "px)")), /*#__PURE__*/React.createElement("input", {
    type: "range",
    min: "1",
    max: "16",
    step: "1",
    ...
    className: "btn sm",
    onClick: onClearOwn
  }, "Clear mine"), isDM && /*#__PURE__*/React.createElement("button", {
    className: "btn sm danger",
    onClick: onClearAll
  }, "Clear all"))));
}
...
function DMWorldPanel({
  ...
  const clearAllPush = () => {
    ...
    dispatch({
      type: 'FORCED_VIEW_PEER_CLEAR_ALL'
    });
  ...
  return /*#__PURE__*/React.createElement(FloatPanel, {
    ...
    className: "float-panel-header"
  }, /*#__PURE__*/React.createElement("span", null, "\uD83C\uDF0D World"), /*#__PURE__*/React.createElement("button", {
    className: "close-x",
    onClick: onClose
  }, "\xD7")), /*#__PURE__*/React.createElement("div", {
    className: "float-panel-body"
    ...
    className: "mono scale-value"
  }, Math.round(tod * 100), "%")), /*#__PURE__*/React.createElement("div", {
    className: "settings-hint"
  }, "Shifts the player view from daylight toward deep night. DM view stays unchanged."), /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      gap: 4,
      marginTop: 6,
    ...
    key: p.label,
    className: `btn sm ${Math.abs(tod - p.v) < 0.03 ? 'active' : ''}`,
    onClick: () => dispatch({
      type: 'TIME_OF_DAY_SET',
      value: p.v
    })
    ...
    className: "settings-label"
  }, "Block Zones ", /*#__PURE__*/React.createElement("span", {
    className: "settings-label-sub"
  }, "\u2014 hide portions of the current map from players")), /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      gap: 6,
      flexWrap: 'wrap'
    ...
    className: `btn sm ${placingBlock ? 'active' : ''}`,
    onClick: onToggleBlockPlace
  }, placingBlock ? '◼ Click-drag…' : '◼ Rectangle'), /*#__PURE__*/React.createElement("button", {
    className: `btn sm ${placingFreeBlock ? 'active' : ''}`,
    onClick: onToggleFreeBlockPlace
  }, placingFreeBlock ? '✎ Drawing…' : '✎ Freeform'), /*#__PURE__*/React.createElement("button", {
    className: `btn sm ${placingCircleBlock ? 'active' : ''}`,
    onClick: onToggleCircleBlockPlace
  }, placingCircleBlock ? '⬤ Click-drag…' : '⬤ Circle'), /*#__PURE__*/React.createElement("button", {
    className: `btn sm ${erasingBlock ? 'danger active' : ''}`,
    onClick: onToggleEraseBlock
  }, erasingBlock ? '✕ Erasing…' : '✕ Eraser'), /*#__PURE__*/React.createElement("button", {
    className: "btn sm danger",
    disabled: !(state.blockZones?.[currentMapId] || []).length,
    onClick: () => {
      if (confirm('Clear all block zones on this map?')) {
        dispatch({
          type: 'BLOCK_ZONE_CLEAR_MAP',
          mapId: currentMapId
        });
      }
    }
    ...
    className: "settings-hint"
  }, (state.blockZones?.[currentMapId] || []).length, " block zone(s). Shapes can overlap. The eraser removes any block it touches while held; clear individual shapes by double-clicking them.")), /*#__PURE__*/React.createElement("div", {
    className: "settings-section"
  }, /*#__PURE__*/React.createElement("label", {
    className: "settings-label"
  }, "Push View"), /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      gap: 6,
      flexWrap: 'wrap',
    ...
    className: `btn sm ${state.forcedView?.mapId === currentMapId ? 'danger active' : ''}`,
    onClick: pushGlobal
  }, state.forcedView?.mapId === currentMapId ? '⚑ Release All' : '⚑ Push to All'), /*#__PURE__*/React.createElement("button", {
    className: "btn sm ghost",
    onClick: clearAllPush
  }, "Clear all pushes")), peers.length === 0 ? /*#__PURE__*/React.createElement("div", {
    className: "settings-hint"
  }, "No players connected.") : /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      flexDirection: 'column',
      gap: 6
    ...
    return /*#__PURE__*/React.createElement("div", {
    ...
    }, claim.playerName || /*#__PURE__*/React.createElement("em", {
      style: {
        color: 'var(--ink-mute)'
      }
    }, "unnamed")), /*#__PURE__*/React.createElement("div", {
      className: "mono",
      style: {
        fontSize: 10,
        color: 'var(--ink-mute)',
    ...
    }, pushed ? `locked → ${maps[pushed.mapId]?.name || '?'}` : 'free')), /*#__PURE__*/React.createElement("select", {
      className: "mono",
      style: {
        padding: '4px 6px',
        fontSize: 11,
      ...
      value: ""
    }, "\u2014 free \u2014"), Object.values(maps).map(m => /*#__PURE__*/React.createElement("option", {
      key: m.id,
      value: m.id
    }, m.name))));
  })))));
...
function DMClaimsPanel({
  ...
  const kickPeer = (peerId, name) => {
    ...
    try {
      sync?.kickPeer(peerId, 'The DM has removed you from the session.');
    } catch {}
    dispatch({
      type: 'DM_KICK_PEER',
      peerId
  ...
  return /*#__PURE__*/React.createElement(FloatPanel, {
    ...
    className: "float-panel-header"
  }, /*#__PURE__*/React.createElement("span", null, "\u2690 Claimed Characters"), /*#__PURE__*/React.createElement("button", {
    className: "close-x",
    onClick: onClose
  }, "\xD7")), /*#__PURE__*/React.createElement("div", {
    className: "float-panel-body"
    ...
    className: "glyph"
  }, "\u2694"), "No players have joined yet.") : /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      flexDirection: 'column',
      gap: 10
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      className: "claim-row-header"
    }, /*#__PURE__*/React.createElement("span", {
      className: "claim-peer-name"
    }, claim.playerName || /*#__PURE__*/React.createElement("em", {
      style: {
        color: 'var(--ink-mute)'
      }
    }, "Unknown player")), /*#__PURE__*/React.createElement("div", {
      style: {
        display: 'flex',
        gap: 6,
        alignItems: 'center'
      }
    }, claim.spectator && /*#__PURE__*/React.createElement("span", {
      className: "claim-badge spectator"
    }, "Spectator"), sync && /*#__PURE__*/React.createElement("button", {
      className: "btn sm danger",
      title: "Kick player",
      onClick: () => kickPeer(peerId, claim.playerName)
    }, "\uD83D\uDEAB Kick"))), /*#__PURE__*/React.createElement("div", {
      className: "claim-peer-id mono"
    }, "id: ", peerId.slice(0, 12), "\u2026"), pc ? /*#__PURE__*/React.createElement("div", {
      className: "claim-entity-row"
    }, /*#__PURE__*/React.createElement("div", {
      className: "entity-swatch",
      style: {
        background: pc.color,
    ...
    }, pc.name), /*#__PURE__*/React.createElement("span", {
      className: "mono",
      style: {
        color: 'var(--ink-mute)',
        fontSize: 11
      }
    }, pc.hp.current, "/", pc.hp.max), /*#__PURE__*/React.createElement("button", {
      className: "btn sm danger",
      onClick: () => {
    ...
    }, "Unclaim")) : !claim.spectator && /*#__PURE__*/React.createElement("div", {
      className: "claim-entity-row",
      style: {
        color: 'var(--ink-mute)',
        fontStyle: 'italic'
      }
    }, "No character claimed"), (claim.familiars || []).map(fid => {
      ...
      }, fam.name), /*#__PURE__*/React.createElement("span", {
        className: "familiar-badge"
      }, "FAM"), /*#__PURE__*/React.createElement("button", {
        className: "btn sm ghost",
        onClick: () => {
          if (confirm(`Release ${fam.name} from this player?`)) {
            dispatch({
              type: 'DM_UNCLAIM_FAMILIAR',
              entityId: fam.id
            });
          }
        }
...
function Breadcrumb({
  ...
  }, chain.map((m, i) => /*#__PURE__*/React.createElement(React.Fragment, {
    key: m.id
...
function DMInterface({
  ...
  const dmVisionSources = useMemo(() => computeVisionSources(state, currentMapId), [state.entities, state.tokens, currentMapId]);
  ...
  useEffect(() => {
    ...
    document.querySelectorAll('.token[data-group-highlight]').forEach(el => el.removeAttribute('data-group-highlight'));
    if (!hoveredGroupId) {
  ...
  const broadcastEphemeralMove = useCallback((tokenId, x, y, mapId) => {
    pendingEphemeralRef.current[tokenId] = {
      x,
      y,
      mapId
    };
  ...
  const deleteCurrentEntity = () => {
    if (!editingEntity || !state.entities[editingEntity.id]) {
      setEditingEntity(null);
      return;
    }
  ...
  const reminderUpsert = r => dispatch({
    type: 'REMINDER_UPSERT',
    peerId: DM_KEY,
    reminder: r
  });
  const reminderDelete = id => dispatch({
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "mode-badge dm"
  }, "\u2694 Dungeon Master"), /*#__PURE__*/React.createElement("span", {
    ...
    className: `btn ${showInit ? 'active' : ''}`,
    onClick: () => setShowInit(!showInit)
  }, "\u2694 Initiative"), /*#__PURE__*/React.createElement("button", {
    className: "btn",
    onClick: () => setShowPresets(true)
  }, "\u2748 Presets"), /*#__PURE__*/React.createElement("button", {
    className: `btn ${showClaims ? 'active' : ''}`,
    onClick: () => setShowClaims(!showClaims)
  }, "\u2690 Claims"), /*#__PURE__*/React.createElement("div", {
    className: "topbar-divider"
  }), /*#__PURE__*/React.createElement("button", {
    className: "btn",
    onClick: () => revealAllOnMap(true)
  }, "\uD83D\uDC41 Reveal All"), /*#__PURE__*/React.createElement("button", {
    className: "btn",
    onClick: () => revealAllOnMap(false)
  }, "\uD83D\uDD76 Hide All"), /*#__PURE__*/React.createElement(ToolsMenu, {
    isDM: true,
    ...
    setPlacingReminder: setPlacingReminder,
    placingBlock: placingBlock,
    setPlacingBlock: setPlacingBlock,
    placingFreeBlock: placingFreeBlock,
    setPlacingFreeBlock: setPlacingFreeBlock,
    placingCircleBlock: placingCircleBlock,
    setPlacingCircleBlock: setPlacingCircleBlock,
    erasingBlock: erasingBlock,
    setErasingBlock: setErasingBlock
  }), /*#__PURE__*/React.createElement("button", {
    className: `btn world-btn ${showWorld ? 'active' : ''}`,
    ...
    return /*#__PURE__*/React.createElement("span", null, "\uD83C\uDF0D ", glyph, " ", /*#__PURE__*/React.createElement("span", {
      className: "world-btn-label"
    }, label));
  })()), /*#__PURE__*/React.createElement("button", {
    className: "btn",
    onClick: longRestAll,
    title: "Restore HP, clear conditions, reset sickness for all party members"
  }, "\u26ED Long Rest"), /*#__PURE__*/React.createElement("div", {
    className: "topbar-spacer"
  }), roomCode && /*#__PURE__*/React.createElement("div", {
    className: "conn-status"
  }, /*#__PURE__*/React.createElement("div", {
    className: `conn-dot ${syncStatus === 'live' ? 'live' : syncStatus === 'connecting' ? 'connecting' : syncStatus === 'error' ? 'error' : ''}`
  }), /*#__PURE__*/React.createElement("span", {
    className: "mono"
  }, roomCode), /*#__PURE__*/React.createElement("span", {
    style: {
      color: 'var(--ink-dim)'
    }
  }, "\xB7 ", peerCount, " ", peerCount === 1 ? 'player' : 'players')), /*#__PURE__*/React.createElement("button", {
    className: "btn",
    ...
    className: "sidebar"
  }, /*#__PURE__*/React.createElement(EntitySidebar, {
    state: state,
    dispatch: dispatch,
    ...
    onHazardUpsert: h => dispatch({
      type: 'HAZARD_UPSERT',
      mapId: state.currentMapId,
      hazard: h
    }),
    onHazardDelete: id => dispatch({
      type: 'HAZARD_DELETE',
      mapId: state.currentMapId,
      id
    }),
    onPlaceHazardDone: () => setPlacingHazard(null),
    onBlockUpsert: zone => dispatch({
      type: 'BLOCK_ZONE_UPSERT',
      mapId: state.currentMapId,
      zone
    }),
    onBlockDelete: id => dispatch({
      type: 'BLOCK_ZONE_DELETE',
      mapId: state.currentMapId,
      id
    }),
    visionSources: dmVisionSources
  }), /*#__PURE__*/React.createElement(TokenTooltip, {
    ...
    className: "canvas-overlay top-left"
  }, /*#__PURE__*/React.createElement(Breadcrumb, {
    map: currentMap,
    maps: state.maps,
    onSwitch: id => dispatch({
      type: 'MAP_SWITCH',
      id
    })
  })), showInit && /*#__PURE__*/React.createElement(InitiativeTracker, {
    state: state,
    dispatch: dispatch,
    mode: "dm",
    onClose: () => setShowInit(false)
  }), showMaps && /*#__PURE__*/React.createElement(MapManager, {
    state: state,
    dispatch: dispatch,
    onClose: () => setShowMaps(false),
    toast: toast
  }), showPresets && /*#__PURE__*/React.createElement(PresetsPanel, {
    state: state,
    dispatch: dispatch,
    onClose: () => setShowPresets(false),
    toast: toast
  }), showClaims && /*#__PURE__*/React.createElement(DMClaimsPanel, {
    state: state,
    dispatch: dispatch,
    sync: sync,
    onClose: () => setShowClaims(false),
    toast: toast
  }), showWorld && /*#__PURE__*/React.createElement(DMWorldPanel, {
    state: state,
    dispatch: dispatch,
    toast: toast,
  ...
  }), showDraw && /*#__PURE__*/React.createElement(DrawingPanel, {
    state: state,
    ...
    isDM: true
  }), showHazards && /*#__PURE__*/React.createElement(HazardsPanel, {
    state: state,
    dispatch: dispatch,
    toast: toast,
  ...
  }), showDice && /*#__PURE__*/React.createElement(DiceTray, {
    state: state,
    ...
    isDM: true,
    dispatch: dispatch,
    onRoll: entry => dispatch({
      type: 'DICE_ROLL',
      entry
    })
  }), showSounds && /*#__PURE__*/React.createElement(SoundboardPanel, {
    state: state,
    dispatch: dispatch,
    toast: toast,
    onClose: () => setShowSounds(false),
    isDM: true,
    onPlay: async soundId => {
      // DM plays a sound: dispatch the event (so all peers see)
      // and load the dataUrl from IDB so we can include it in
      // the broadcast for peers that don't yet have it cached.
      let dataUrl = null,
        name = null;
      try {
      ...
      const ev = {
        ...
        soundId,
        action: 'play',
        // Inline the dataUrl so players can play immediately even
        // if they've never heard this sound before. They'll cache
        // it locally for subsequent plays.
        dataUrl,
        name
      };
      dispatch({
  ...
  }), showGroups && /*#__PURE__*/React.createElement(GroupsPanel, {
    state: state,
    dispatch: dispatch,
    toast: toast,
  ...
  }), selectedToken && selectedTokenEntity && /*#__PURE__*/React.createElement(TokenDetailPanel, {
    state: state,
    token: selectedToken,
    ...
    return /*#__PURE__*/React.createElement("div", {
    ...
    }), /*#__PURE__*/React.createElement("span", null, ent?.name || 'Token')), /*#__PURE__*/React.createElement("button", {
      className: "token-ctx-item",
      onClick: () => {
        dispatch({
          type: 'TOKEN_VISIBILITY',
          id: t.id,
      ...
      className: "ctx-icon"
    }, t.visible ? '🕶' : '👁'), t.visible ? 'Hide from players' : 'Reveal to players'), /*#__PURE__*/React.createElement("button", {
      className: "token-ctx-item",
      onClick: () => {
      ...
      className: "ctx-icon"
    }, "\u25C8"), "Open details"), ent && /*#__PURE__*/React.createElement("button", {
      className: "token-ctx-item",
      onClick: () => {
      ...
      className: "ctx-icon"
    }, "\u270E"), "Edit entity"), /*#__PURE__*/React.createElement("div", {
      className: "token-ctx-sep"
    }), /*#__PURE__*/React.createElement("button", {
      className: "token-ctx-item danger",
      onClick: () => {
        if (confirm('Remove this token from the map?')) dispatch({
          type: 'TOKEN_REMOVE',
          id: t.id
        });
        closeCtxMenu();
  ...
  })(), editingEntity && /*#__PURE__*/React.createElement("div", {
    className: "modal-overlay",
    onClick: e => e.target === e.currentTarget && setEditingEntity(null)
  }, /*#__PURE__*/React.createElement("div", {
    className: "modal slide-up"
  }, /*#__PURE__*/React.createElement("div", {
    className: "float-panel-header"
  }, /*#__PURE__*/React.createElement("span", null, state.entities[editingEntity.id] ? '✎ Edit Entity' : '＋ New Entity'), /*#__PURE__*/React.createElement("button", {
    className: "close-x",
    onClick: () => setEditingEntity(null)
  }, "\xD7")), /*#__PURE__*/React.createElement("div", {
    className: "float-panel-body"
  }, /*#__PURE__*/React.createElement(EntityForm, {
    initial: editingEntity,
    onSave: saveEntity,
    onCancel: () => setEditingEntity(null)
  }), state.entities[editingEntity.id] && /*#__PURE__*/React.createElement("div", {
    style: {
      marginTop: 12,
      paddingTop: 12,
      borderTop: '1px solid var(--border-soft)',
    ...
    className: "btn",
    onClick: duplicateCurrentEntity,
    title: "Create a copy of this entity"
  }, "\u2398 Duplicate"), /*#__PURE__*/React.createElement("button", {
    className: "btn danger",
    onClick: deleteCurrentEntity
  }, "Delete Entity")))))), showSettings && /*#__PURE__*/React.createElement(SettingsModal, {
    settings: settings,
    onChange: onSettingsChange,
    onClose: onCloseSettings,
    mode: "dm",
    mapScale: state.mapScale || 1,
    onMapScaleChange: v => dispatch({
      type: 'MAP_SCALE_SET',
      scale: v
    })
  }));
...
function PartySidebar({
  ...
  const partyMembers = Object.values(state.entities).filter(e => (e.type === 'PC' || e.type === 'Familiar') && entityIdsOnMap.has(e.id))
  ...
  return /*#__PURE__*/React.createElement("div", {
    className: "sidebar player-sidebar left"
  }, /*#__PURE__*/React.createElement("div", {
    className: "sidebar-section"
  }, /*#__PURE__*/React.createElement("div", {
    className: "sidebar-title"
  }, /*#__PURE__*/React.createElement("span", null, "\u269C The Party"))), /*#__PURE__*/React.createElement("div", {
    className: "sidebar-section grow"
  }, /*#__PURE__*/React.createElement("div", {
    className: "party-list"
  }, partyMembers.length === 0 ? /*#__PURE__*/React.createElement("div", {
    className: "empty-state"
  }, /*#__PURE__*/React.createElement("span", {
    className: "glyph"
  }, "\u2726"), "No party members yet.") : partyMembers.map(e => {
    const hpPct = e.hp.max > 0 ? e.hp.current / e.hp.max * 100 : 0;
    const hpClass = hpPct <= 25 ? 'critical' : hpPct <= 50 ? 'low' : '';
    ...
    }, e.name, isYou && e.id === claimedEntityId && /*#__PURE__*/React.createElement("span", {
      className: "own-pc-badge"
    }, "YOU"), isYou && isFamiliar && /*#__PURE__*/React.createElement("span", {
      className: "familiar-badge"
    }, "YOURS"), isFamiliar && !isYou && /*#__PURE__*/React.createElement("span", {
      className: "familiar-badge dim"
    }, "FAM")), /*#__PURE__*/React.createElement("div", {
      className: "party-meta mono"
    }, isFamiliar ? e.faction ? `bond: ${e.faction}` : 'Familiar' : `L${e.level} ${e.class || ''}`), /*#__PURE__*/React.createElement("div", {
      className: "party-hp-row"
    }, /*#__PURE__*/React.createElement("div", {
      className: "party-hp-bar"
    }, /*#__PURE__*/React.createElement("div", {
      className: `party-hp-fill ${hpClass}`,
      style: {
        width: `${hpPct}%`
      }
    })), /*#__PURE__*/React.createElement("span", {
      className: `party-hp-text mono ${hpClass}`
    }, e.hp.current, "/", e.hp.max)), e.conditions.length > 0 && /*#__PURE__*/React.createElement("div", {
      className: "party-conditions"
    }, e.conditions.slice(0, 6).map(c => /*#__PURE__*/React.createElement("span", {
      key: c,
      className: "party-cond-pill",
      style: {
        background: CONDITION_COLORS[c] || '#555'
...
function RevealedMonstersSidebar({
  state,
  currentMapId
}) {
  ...
  const revealedFoes = useMemo(() => {
    ...
    for (const t of Object.values(state.tokens)) {
      ...
      if (!byId.has(ent.id)) byId.set(ent.id, {
        entity: ent,
        tokens: []
      });
  ...
  return /*#__PURE__*/React.createElement("div", {
    className: "sidebar player-sidebar right"
  }, /*#__PURE__*/React.createElement("div", {
    className: "sidebar-section"
  }, /*#__PURE__*/React.createElement("div", {
    className: "sidebar-title"
  }, /*#__PURE__*/React.createElement("span", null, "\u2756 Revealed"))), /*#__PURE__*/React.createElement("div", {
    className: "sidebar-section grow"
  }, /*#__PURE__*/React.createElement("div", {
    className: "revealed-list"
  }, revealedFoes.length === 0 ? /*#__PURE__*/React.createElement("div", {
    className: "empty-state"
  }, /*#__PURE__*/React.createElement("span", {
    className: "glyph"
  }, "\u2756"), "Nothing revealed yet.") : revealedFoes.map(({
    entity: e,
    tokens
  }) => {
    const hpPct = e.hp.max > 0 ? e.hp.current / e.hp.max * 100 : 0;
    ...
    return /*#__PURE__*/React.createElement("div", {
      key: e.id,
      className: `revealed-card revealed-type-${swatchClass}`
    }, /*#__PURE__*/React.createElement("div", {
      className: "revealed-header"
    }, /*#__PURE__*/React.createElement("div", {
      className: `entity-swatch ${swatchClass}`,
      style: {
        background: e.color
      ...
      className: "revealed-type-badge"
    }, e.type), e.playerDescription ? /*#__PURE__*/React.createElement("div", {
      className: "revealed-desc"
    }, e.playerDescription) : /*#__PURE__*/React.createElement("div", {
      className: "revealed-desc",
      style: {
        fontStyle: 'italic',
        color: 'var(--ink-mute)'
      }
    }, "A creature of uncertain nature."), e.conditions.length > 0 && /*#__PURE__*/React.createElement("div", {
      className: "revealed-conditions"
    }, e.conditions.map(c => /*#__PURE__*/React.createElement("span", {
      key: c,
      className: "party-cond-pill",
      style: {
        background: CONDITION_COLORS[c] || '#555'
      }
    }, c))), tokens.length > 1 && /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: 10,
        color: 'var(--ink-mute)',
        marginTop: 4,
...
function PlayerInterface({
  ...
  const myClaim = state.claims?.[myPeerId] || {
    pc: null,
    familiars: [],
    playerName: '',
    spectator: false
  ...
  // bonded familiars.
  const visionOwned = useMemo(() => Array.from(ownedEntityIds).map(id => state.entities[id]).filter(Boolean), [ownedEntityIds, state.entities]);
  ...
  const allClaimedPCIds = new Set(Object.values(state.claims || {}).map(c => c.pc).filter(Boolean));
  const allClaimedFamiliarIds = new Set(Object.values(state.claims || {}).flatMap(c => c.familiars || []));
  ...
  if (!hasMadeChoice && syncStatus === 'live') {
    ...
    }, "\xB7 ", playerName)), /*#__PURE__*/React.createElement("button", {
      className: "btn ghost",
      onClick: onLogout
    }, "\u238B Leave")), /*#__PURE__*/React.createElement(PlayerOnboardingGate, {
      state: state,
      myPeerId: myPeerId,
      playerName: playerName,
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "topbar-divider"
  }), claimedEntity ? /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      alignItems: 'center',
      gap: 8
    ...
    className: "entity-swatch",
    style: {
      background: claimedEntity.color,
      width: 12,
      height: 12
    }
  }), /*#__PURE__*/React.createElement("span", {
  ...
  }, claimedEntity.name), /*#__PURE__*/React.createElement("span", {
    className: "mono",
    style: {
      fontSize: 11,
      color: 'var(--ink-dim)'
    }
  }, claimedEntity.hp.current, "/", claimedEntity.hp.max, " HP"), /*#__PURE__*/React.createElement("button", {
    className: "btn sm primary",
    onClick: () => setShowSheet(true)
  }, "\u25C8 Edit My Sheet"), (availableFamiliars.length > 0 || (myClaim.familiars || []).length > 0) && /*#__PURE__*/React.createElement("button", {
    className: "btn sm",
    onClick: () => setShowClaim(true),
    title: "Claim or release a familiar"
  }, "\u2726 Familiar", (myClaim.familiars || []).length > 0 ? `s (${myClaim.familiars.length})` : ''), /*#__PURE__*/React.createElement("button", {
    className: "btn sm ghost",
    onClick: unclaimPC
  }, "Release")) : myClaim.spectator ? /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      alignItems: 'center',
      gap: 8
  ...
  }, "\uD83D\uDC41 Spectator mode"), /*#__PURE__*/React.createElement("button", {
    className: "btn sm",
    onClick: () => setShowClaim(true)
  }, "\u2690 Claim Character")) : /*#__PURE__*/React.createElement("button", {
    className: "btn primary",
    onClick: () => setShowClaim(true)
  }, "\u2690 Claim Character"), /*#__PURE__*/React.createElement("div", {
    className: "topbar-divider"
  }), /*#__PURE__*/React.createElement("button", {
    className: `btn ${showInit ? 'active' : ''}`,
    onClick: () => setShowInit(!showInit)
  }, "\u2694 Initiative"), /*#__PURE__*/React.createElement(ToolsMenu, {
    isDM: false,
    measureMode: measureMode,
    setMeasureMode: setMeasureMode,
    showDraw: showDraw,
    ...
    className: "main player-view"
  }, /*#__PURE__*/React.createElement(PartySidebar, {
    state: state,
    ...
    className: `canvas-container sick-level-${sicknessLevel} tod-${Math.round((state.timeOfDay || 0) * 10)} ${claimedEntity && claimedEntity.hp.current <= 0 ? 'downed' : ''}`
  }, /*#__PURE__*/React.createElement(MapCanvas, {
    map: currentMap,
    entities: state.entities,
    tokens: state.tokens,
    ...
    drawWidth: drawWidth,
    drawOwner: myPeerId,
    onDrawingUpsert: d => playerActionSender({
      type: 'drawing_upsert',
      payload: {
        mapId: currentMapId,
        drawing: d
    ...
    x: cursorPos.x,
    y: cursorPos.y
  }), sicknessLevel > 0 && /*#__PURE__*/React.createElement("div", {
    className: `sickness-vignette sick-level-${sicknessLevel}`,
    "aria-hidden": "true"
  }), /*#__PURE__*/React.createElement("div", {
    className: "canvas-overlay top-left"
  }, currentMap && /*#__PURE__*/React.createElement(Breadcrumb, {
    map: currentMap,
    maps: state.maps,
    onSwitch: () => {}
  })), isForced && /*#__PURE__*/React.createElement("div", {
    className: "canvas-overlay bottom-center"
  }, /*#__PURE__*/React.createElement("div", {
    className: "forced-view-banner"
  }, /*#__PURE__*/React.createElement("span", {
    className: "glyph"
  }, "\u2691"), "DM-controlled view \xB7 ", currentMap?.name)), syncStatus !== 'live' && /*#__PURE__*/React.createElement("div", {
    className: "canvas-overlay bottom-center"
  }, /*#__PURE__*/React.createElement("div", {
    className: "forced-view-banner"
  }, syncStatus === 'connecting' ? 'Connecting to the table…' : syncStatus === 'error' ? 'Connection lost. Reopen the page to retry.' : 'Offline')), showInit && /*#__PURE__*/React.createElement(InitiativeTracker, {
    state: state,
    dispatch: () => {},
    mode: "player",
    onClose: () => setShowInit(false)
    ...
    isDM: false,
    dispatch: () => {},
    onRoll: entry => playerActionSender({
      type: 'dice_roll',
      payload: {
        entry
      }
    ...
    claimedEntityId: claimedEntityId,
    playerActionSender: playerActionSender
  }), showClaim && /*#__PURE__*/React.createElement("div", {
    className: "modal-overlay",
    ...
    className: "float-panel-header"
  }, /*#__PURE__*/React.createElement("span", null, "\u2690 Claim"), /*#__PURE__*/React.createElement("button", {
    className: "close-x",
    onClick: () => setShowClaim(false)
  }, "\xD7")), /*#__PURE__*/React.createElement("div", {
    className: "float-panel-body"
  }, !claimedEntity && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("label", null, "Characters"), unclaimedPCs.length === 0 ? /*#__PURE__*/React.createElement("div", {
    className: "empty-state"
  }, /*#__PURE__*/React.createElement("span", {
    className: "glyph"
  }, "\u2694"), "No unclaimed characters.") : /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      flexDirection: 'column',
      gap: 6,
  ...
  }, unclaimedPCs.map(e => /*#__PURE__*/React.createElement("div", {
    key: e.id,
    ...
    className: "btn primary sm"
  }, "Claim"))))), availableFamiliars.length > 0 && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("label", null, "Familiars ", /*#__PURE__*/React.createElement("span", {
    style: {
      color: 'var(--ink-mute)',
      fontWeight: 400,
      textTransform: 'none',
  ...
  }, "\u2014 you may claim multiple")), /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      flexDirection: 'column',
      gap: 6
    }
  }, availableFamiliars.map(e => /*#__PURE__*/React.createElement("div", {
    key: e.id,
    className: "claim-option familiar",
    onClick: () => claimFamiliar(e.id)
  }, /*#__PURE__*/React.createElement("div", {
    className: "pc-avatar familiar-avatar",
    style: {
      background: e.color,
      width: 32,
      height: 32
  ...
  }, "Familiar ", e.faction ? `· bonded to ${e.faction}` : '')), /*#__PURE__*/React.createElement("button", {
    className: "btn sm"
  }, "Claim"))))), claimedFamiliarIds.length > 0 && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("label", {
    style: {
      marginTop: 14
    }
  }, "Your familiars"), /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      flexDirection: 'column',
      gap: 4
    ...
    return /*#__PURE__*/React.createElement("div", {
    ...
    }, (f.name[0] || '?').toUpperCase()), /*#__PURE__*/React.createElement("span", {
      style: {
        flex: 1,
        fontSize: 13
      }
      ...
      className: "btn sm ghost",
      onClick: () => unclaimFamiliar(fid)
    }, "Release"));
  })))))), showSheet && /*#__PURE__*/React.createElement(EditMySheetModal, {
    state: state,
    myPeerId: myPeerId,
    claim: myClaim,
    playerActionSender: playerActionSender,
    onClose: () => setShowSheet(false)
  })), /*#__PURE__*/React.createElement(RevealedMonstersSidebar, {
    state: state,
    currentMapId: currentMapId
...
function Root() {
  const [auth, setAuth] = useState(() => {
    ...
    if (loaded && loaded.mode === 'player' && !loaded.playerId) {
      ...
      try {
        localStorage.setItem(AUTH_KEY, JSON.stringify(loaded));
      } catch {}
    }
    return loaded;
  });
...
function Session({
        ...
        if (loaded) {
          ...
          if (raw) {
            try {
              ...
              persistSessionToIDB(migrated).catch(e => console.warn('[plagues-call] initial IDB write failed:', e?.message));
            } catch (e) {
  ...
  const persistNow = useCallback((reason = 'routine') => {
    ...
    const t0 = performance.now();
    persistSessionToIDB(snapshot).then(({
      jsonBytes,
      imageCount
    }) => {
  ...
  useEffect(() => {
    ...
    const structSig = [Object.keys(s.tokens || {}).length, Object.keys(s.entities || {}).length, Object.keys(s.maps || {}).length, s.currentMapId].join('::');
  ...
  useEffect(() => {
    ...
    const sync = new SyncManager({
      ...
      onSoundData: (soundId, name, dataUrl) => {
        ...
        idbSet(IDB_STORES.sounds, soundId, {
          id: soundId,
          name,
          dataUrl,
          ts: Date.now()
    ...
    // v7.2: join start/complete logging so we can verify that
    // "claim modal ready" latency has actually dropped.
    ...
    if (auth.mode === 'dm') {
      ...
      // One-shot listener: log when the first state_update arrives
      // (= claim modal is ready to render).
      const onFirstState = () => {
        const elapsed = performance.now() - joinT0;
        console.log(`[plagues-call] claim modal ready in ${elapsed.toFixed(0)}ms`);
      };
  ...
  const handlePlayerAction = useCallback((action, peerId) => {
    ...
    const ownedByPeer = (s, pid) => {
      ...
      // v3/v5: bonded familiars. v5 adds bondedPcId (bond by PC, not peer).
      for (const [eid, ent] of Object.entries(s.entities)) {
        if (!ent || ent.type !== 'Familiar') continue;
        if (ent.bondedPeerId === pid) out.add(eid);
        if (c && ent.bondedPcId && ent.bondedPcId === c.pc) out.add(eid);
      }
      return out;
    };
    ...
    const PLAYER_FIELD_WHITELIST = new Set(['name', 'color', 'ac', 'speed', 'initBonus', 'passivePerception', 'class', 'level', 'playerName', 'notes', 'playerDescription', 'imageUrl', 'faction', 'role',
    // v4 fix #9: vision stats player-editable (DM can still override)
    'darkvision', 'lightRadius']);
      ...
      case 'claim_pc':
        {
          ...
          const takenBySomeoneElse = Object.entries(curr.claims || {}).some(([k, c]) => c.pc === entityId && k !== peerId);
      ...
      case 'claim_familiar':
        {
          ...
          const takenBySomeoneElse = Object.entries(curr.claims || {}).some(([k, c]) => (c.familiars || []).includes(entityId) && k !== peerId);
          if (takenBySomeoneElse) return;
          dispatch({
            type: 'CLAIM_FAMILIAR',
            peerId,
      ...
      case 'drawing_upsert':
        {
          ...
          if (drawing.type === 'free') {
            ...
            safe = {
              id: uid('draw_'),
              type: 'line',
              x0: Number(drawing.x0) || 0,
              y0: Number(drawing.y0) || 0,
              x1: Number(drawing.x1) || 0,
              y1: Number(drawing.y1) || 0,
              color,
              width,
              owner: peerId
            };
            ...
            safe = {
              id: uid('draw_'),
              type: 'circle',
              cx: Number(drawing.cx) || 0,
              cy: Number(drawing.cy) || 0,
      ...
      case 'dice_roll':
        {
          ...
          if (Array.isArray(e.groups)) {
            ...
            for (const g of e.groups) {
              ...
              const sanitized = g.results.slice(0, 100).map(r => clamp(r | 0, 1, g.die | 0));
  ...
  if (auth.mode === 'dm') {
    return /*#__PURE__*/React.createElement(DMInterface, {
      state: state,
      dispatch: dispatch,
  ...
  return /*#__PURE__*/React.createElement(PlayerInterface, {
    state: state,
    myPeerId: myPeerId,
...
root.render(/*#__PURE__*/React.createElement(ToastProvider, null, /*#__PURE__*/React.createElement(Root, null)));